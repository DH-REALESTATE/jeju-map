		...
		try {
			...
			try {
				...
				if (isRealjejuKakaoUrlColumnError(kakaoError) || isRealjejuMissingKakaoRpcError(kakaoError)) {
					return openAuthErrorModal("카카오 오픈 채팅방 저장 기능이 아직 DB에 설치되지 않았습니다.\nsql/admin_tools_3.104.sql을 Supabase SQL Editor에서 실행해 주세요.", "중개사 정보", kakaoInput || officeNameInput);
				}
...
function applyLoggedInAccountUI(user, profile)
		...
		brokerOfficeEditPageForm.addEventListener("submit", async (e) => {
			...
			if (!regNo) return openAuthErrorModal("등록번호를 입력하세요.", "중개사무소 수정", regNoInput);
				...
				try {
					...
					if (isRealjejuKakaoUrlColumnError(kakaoError) || isRealjejuMissingKakaoRpcError(kakaoError)) {
						return openAuthErrorModal("카카오 오픈 채팅방 저장 기능이 아직 DB에 설치되지 않았습니다.\nsql/admin_tools_3.104.sql을 Supabase SQL Editor에서 실행해 주세요.", "중개사무소 수정", kakaoInput || officeNameInput);
					}
			...
			if (!licenseNo) return openAuthErrorModal("등록번호를 입력하세요.", "중개사무소 가입 신청", document.getElementById("brokerLicenseNoInput"));
				...
				if (isEditMode) {
					...
					try {
						...
						console.error("중개사무소 카카오 오픈 채팅방 저장 실패:", kakaoError);
						if (isRealjejuKakaoUrlColumnError(kakaoError) || isRealjejuMissingKakaoRpcError(kakaoError)) {
							return openAuthErrorModal("카카오 오픈 채팅방 저장 기능이 아직 DB에 설치되지 않았습니다.\nsql/admin_tools_3.104.sql을 Supabase SQL Editor에서 실행해 주세요.", "중개사무소 정보", document.getElementById("brokerOfficeKakaoInput"));
						}
	...
	{
		return "sql/admin_tools_3.104.sql을 Supabase SQL Editor에서 실행한 뒤 다시 시도하세요.";
	}
					...
					return `
						<article class="my-suite-inquiry-item admin-inquiry-item ${deleted ? "deleted" : (answered ? "answered" : "pending")}" data-admin-inquiry-id="${id}">
							<div class="my-suite-inquiry-row admin-inquiry-row">
								<button type="button" class="admin-inquiry-main-button" data-admin-inquiry-toggle="${id}">
									<span class="my-suite-inquiry-date admin-inquiry-date">${escapeAdminHtml(formatAdminInquiryDate(row.created_at))}</span>
									<span class="admin-inquiry-main">
										<span class="my-suite-inquiry-title admin-inquiry-title">${escapeAdminHtml(row.title || "제목 없음")}</span>
										<span class="admin-inquiry-author">${escapeAdminHtml(author)}</span>
									</span>
								</button>
	...
	function renderAdminUsers(rows)
	{
		...
		` + filteredRows.map((row) => {
			...
			const canForceWithdraw = rowId && !isAdminProfileRow(row) && !withdrawn;
			return `
				...
				<div class="broker-listing-row admin-listing-row" data-admin-listing-id="${rowId}">
					...
					<div class="broker-listing-menu-cell">
						...
						<div class="broker-listing-more-menu" data-admin-listing-menu-panel="${rowId}">
							...
							<button type="button" class="broker-listing-menu-item" data-admin-listing-action="closed" data-listing-id="${rowId}">거래완료</button>
							<button type="button" class="broker-listing-menu-item" data-admin-listing-action="hidden" data-listing-id="${rowId}">숨김</button>
