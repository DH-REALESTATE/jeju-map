/* REALJEJU 5.422 ordered runtime tail. */
...
(function bindAuthStateGoHome()
{
	...
	function shouldPreserveCleanRouteOnAuthChange()
	{
		...
		return /^\/(?:properties(?:\/[^/]+)?|companies|jobs|used|cars|meetups|notices|mypage\/(?:profile|office|payments|inquiries)|admin(?:\/(?:notices|inquiries|coupons|users|broker-applications|properties))?)$/i.test(pathname);
	}
...
(function bindRealjejuRouteController()
{
	...
	function normalizeRoutePath(pathname)
	{
		...
		const routePath = normalized.replace(
			/^\/menu_routes(?=\/(?:properties|companies|jobs|used|cars|meetups|notices|mypage|admin)(?:\/|$))/i,
			""
		);
		return routePath.replace(
			/^\/((?:properties|companies|jobs|used|cars|meetups|notices|mypage|admin)(?:\/[^/]+)*)\/index\.html$/i,
			"/$1"
		);
	}
	...
	function shouldDeferRouteUntilAppReady(route)
	{
		...
		return route.type === "category" && route.category !== "part-time";
	}
...
// REALJEJU 5.422: 필지·실거래·건축물 상세 기능은 property-info 모듈로 분리했습니다.
