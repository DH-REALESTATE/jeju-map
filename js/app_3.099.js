...
const REALJEJU_BROKER_ROLE_VALUES = new Set(["broker", "agent", "agent_sub", "agent_staff", "corporation"]);
...
const VIEW_INCREMENT_DEDUPE_MS = 8000;
...
function scheduleSupabaseScriptLoad()
{
	const run = () => {
		...
		.then(() => {
			...
			if (typeof window.realjejuReloadMapListings === "function") {
...
function getMapListingArea(row)
{
	...
	const areaMode = String(areas.area_mode || payload.areaMode || "").trim();
	const primaryLabel = areaMode === "contract_private" || /오피스텔|호텔|펜션/.test(typeText) ? "계약" : "공급";
	const parts = [];
	if (/토지|임야|대지/.test(typeText) && areas.land_m2) parts.push(`${areas.land_m2}㎡`);
	if (areas.supply_m2) parts.push(`${primaryLabel}${areas.supply_m2}㎡`);
	if (areas.exclusive_m2) parts.push(`전용${areas.exclusive_m2}㎡`);
	if (!parts.length && areas.land_m2) parts.push(`${areas.land_m2}㎡`);
	return parts.join("ㆍ") || "-";
}
...
{
	const payload = row && row.payload && typeof row.payload === "object" ? row.payload : {};
	const storedNo = String(row?.listing_no || payload.listing_no || "").trim();
	if (storedNo) return storedNo;
	const source = [row?.title, payload.description, payload.agency_memo].filter(Boolean).join("\n");
	const bracketMatch = source.match(/\(([^)]*\d[^)]*)\)/);
	if (bracketMatch) return bracketMatch[1].trim().replace(/\s+/g, "");
	...
	return [
		{
			...
			rows: [
				...
				{ label: "전용 면적", value: areas.exclusive_m2 ? `${areas.exclusive_m2}㎡` : "" },
				{ label: "공급 면적", value: areas.supply_m2 ? `${areas.supply_m2}㎡` : "" },
				{ label: "대지 면적", value: areas.land_m2 ? `${areas.land_m2}㎡` : "" }
			]
		},
		{
			title: "매물 정보",
...
async function loadProperties()
{
	try {
		...
		const normalized = rows
			.filter(row => {
				const effectiveStatus = row?.payload && typeof row.payload === "object" ? row.payload.broker_status : "";
				return effectiveStatus !== "closed" && Number.isFinite(Number(row?.lat)) && Number.isFinite(Number(row?.lng));
			})
			.map(row => mapPropertyListingRowToMapItem(row, ownerData.profilesByUserId, ownerData.agenciesById, ownerData.agenciesByUserId, ownerData.agenciesByOfficeName))
...
window.realjejuReloadMapListings = loadProperties;
...
function getCardRegionName(value)
{
	...
	const matches = text.match(/[가-힣0-9]+(?:읍|면|동|리)/g);
		...
		if (String(dbItem.agentOffice || "").trim() === "개인 매물 · 직거래" || String(dbItem.dealMethod || "").trim() === "direct") {
			try {
				...
				if (listingUserId) {
					...
					const agency = agencies.find(candidate => candidate && candidate.status === "active") || agencies[0] || null;
					let profile = null;
					try {
...
function renderDetailSimilarListings(detailItem)
{
	if (!detailSimilarCard || !detailSimilarGrid) return;
	const items = getDetailSimilarItems(detailItem);
	if (!items.length) {
...
{
	const { syncUrl = true, replaceUrl = false, forceOpen = false } = options || {};
...
function renderList(data)
{
	...
	propertyList.innerHTML = data.map(item => {
		...
		const regionText = escapeHtml(getCardRegionName(item.address));
...
function normalizeRealjejuPhone(value)
	...
	function isValidRealjejuMobilePhone(value)
	{
		...
		function isRealjejuMissingKakaoRpcError(error)
		{
			...
			return /save_my_agency_kakao_url|PGRST202|schema cache|function/i.test(text);
		}
	...
	async function fetchMySuiteProfileAndBrokerOffice()
	{
		...
		try {
			const { data: officeRows } = await client
				...
				.order("created_at", { ascending: false })
				.limit(10);
			brokerOffice = pickCurrentBrokerOffice(officeRows);
		} catch (err) {
			brokerOffice = null;
		}
	...
	{
		document.body.classList.remove("my-suite-page-open");
	...
	function renderMySuiteFavoritesContent()
		{
			...
			const favorites = getFavoriteListingItems(mySuiteFavoriteView);
			...
			content.innerHTML = `
				<div class="my-suite-favorites-filterbar">
					...
					<div class="broker-home-filter-item">
						<button type="button" class="broker-home-filter-btn">매물 유형 <i class="fa-solid fa-chevron-down" aria-hidden="true"></i></button>
					</div>
					<div class="broker-home-filter-item">
						<button type="button" class="broker-home-filter-btn">거래 유형 <i class="fa-solid fa-chevron-down" aria-hidden="true"></i></button>
					</div>
							...
							<div class="broker-listing-row my-suite-favorite-row" data-favorite-listing-id="${rowId}">
								...
								<div class="broker-listing-menu-cell">
									...
									<div class="broker-listing-more-menu" data-favorite-menu-panel="${rowId}">
										...
										` : `
										<button type="button" class="broker-listing-menu-item" data-favorite-action="inquiry" data-listing-id="${rowId}">1:1 문의내용</button>
	...
	async function renderMySuiteProfileContent()
	{
		...
		{
			const value = String(role || "").trim();
			return value === "broker" || value === "agent" || value === "agent_sub" || value === "agent_staff" || value === "corporation" || value === "중개사" || value === "공인중개사";
		}
		...
		function safeBrokerStatus(profile, brokerOffice)
		{
			...
			if (brokerOffice.status === "new") return "가입 신청 진행중";
		...
		function renderProfileCard(user, profile, brokerOffice)
		{
			...
			const displayName = profile && profile.name ? profile.name : "이름 미등록";
			...
			const isApplicationWaiting = brokerOffice && (brokerOffice.status === "new" || brokerOffice.status === "pending");
			const brokerActionLabel = brokerOffice && brokerOffice.status === "active"
				? "중개사 정보"
				: (isApplicationWaiting ? "가입 신청 진행중" : "중개사무소 가입 신청");
			const brokerAction = brokerOffice && brokerOffice.status === "active"
				? "broker-office-info"
	...
	async function renderMySuiteBrokerOfficeContent()
	{
		...
		const statusLabel = (status) => {
			...
			if (status === "new") return "가입 신청 진행중";
		...
		const fetchDetailedBrokerOffice = async (user, currentOffice) => {
			const client = getRealjejuSupabaseClient();
			...
			try {
				...
				const detailed = (currentId ? list.find((row) => row && String(row.id || "") === currentId) : null)
					|| list.find((row) => row && row.status === "active" && (row.office_reg_no || row.office_address || row.email))
					|| list.find((row) => row && (row.office_reg_no || row.office_address || row.email))
					|| list.find((row) => row && row.status === "active")
					|| list[0]
					|| null;
		...
		try {
			...
			const detailedBrokerOffice = await fetchDetailedBrokerOffice(user, brokerOffice || window.realjejuCurrentBrokerOffice || null);
			window.realjejuCurrentAuthUser = user;
			window.realjejuCurrentProfile = profile || null;
	...
	function openMySuitePage(tabKey)
	{
		...
		if (key === "favorites") mySuiteFavoriteView = "active";
		const panel = document.getElementById("mySuitePanel");
		if (!panel) return;
	...
	document.addEventListener("click", async (e) => {
		...
		if (favoriteActionBtn) {
			...
			if (action === "delete") {
				...
				if (action === "restore") {
					await restoreFavoriteListingFromTrash(listingId);
					renderRealjejuMySuiteFavoritesContent();
					syncFavoriteHeartButtons();
					return;
			}
			if (action === "inquiry") {
				...
				openAuthErrorModal("1:1 문의 기능은 준비중입니다.", "1:1 문의내용", favoriteActionBtn);
				return;
			}
			if (action === "report") {
				favoriteActionBtn.disabled = false;
	...
	function renderMySuiteInlineProfileEditPage(profile, user)
	{
		...
		content.innerHTML = `
			<form class="myinfo-page-card my-suite-myinfo-card" id="mySuiteProfileEditForm">
				<div class="myinfo-page-photo-box my-suite-myinfo-photo-box profile-suite-photo-control">
					<img src="${safeHtml(currentProfileImage || fallbackImage)}" alt="프로필 사진" class="myinfo-page-photo" id="mySuiteProfileEditImage">
					<button type="button" class="profile-suite-photo-btn" data-my-suite-action="profile-photo">사진 선택</button>
	...
	async function saveMySuiteProfileEdit()
	{
		...
		try {
			...
			window.realjejuCurrentAuthUser = user;
			window.realjejuCurrentProfile = savedProfile;
			mySuiteProfilePhotoFile = null;
			mySuiteProfilePhotoUrl = profileImage;
			window.authProfilePhotoUrl = profileImage;
			if (typeof writeRealjejuCachedProfile === "function") writeRealjejuCachedProfile(user.id, savedProfile);
			renderMySuiteProfileContent();
		} catch (err) {
	...
	async function openMySuiteBrokerOfficeEdit()
	{
		...
		try {
			const loaded = await fetchMySuiteProfileAndBrokerOffice();
			if (loaded && loaded.user) {
				window.realjejuCurrentAuthUser = loaded.user;
				window.realjejuCurrentProfile = loaded.profile || null;
	...
	async function saveMySuiteBrokerOfficeEdit()
	{
		...
		try {
			...
			if (error) {
				...
				if (isRealjejuKakaoUrlColumnError(error)) return openAuthErrorModal(REALJEJU_KAKAO_URL_COLUMN_MESSAGE, "중개사 정보", kakaoInput || officeNameInput);
				return openAuthErrorModal("중개사 정보 저장에 실패했습니다.", "중개사 정보", officeNameInput);
			}
			const updatedOffice = pickMySuiteUpdatedBrokerOfficeRow(updatedRows, agencyId);
			let savedKakaoRow = null;
			try {
				savedKakaoRow = await saveRealjejuAgencyKakaoUrl(client, (updatedOffice && updatedOffice.id) || agencyId, kakaoUrl);
			} catch (kakaoError) {
				console.error("마이페이지 카카오 오픈 채팅방 저장 실패:", kakaoError);
				if (isRealjejuKakaoUrlColumnError(kakaoError) || isRealjejuMissingKakaoRpcError(kakaoError)) {
					return openAuthErrorModal("카카오 오픈 채팅방 저장 기능이 아직 DB에 설치되지 않았습니다.\nsql/admin_tools_3.084.sql을 Supabase SQL Editor에서 실행해 주세요.", "중개사 정보", kakaoInput || officeNameInput);
				}
				return openAuthErrorModal("카카오 오픈 채팅방 저장에 실패했습니다.", "중개사 정보", kakaoInput || officeNameInput);
			}
			window.realjejuCurrentBrokerOffice = {
				...(window.realjejuCurrentBrokerOffice || {}),
				...(updatedOffice || {}),
				id: (savedKakaoRow && savedKakaoRow.id) || (updatedOffice && updatedOffice.id) || agencyId,
				...
				phone: phone,
				email: email,
				kakao_url: (savedKakaoRow && savedKakaoRow.kakao_url) || kakaoUrl,
				updated_at: (savedKakaoRow && savedKakaoRow.updated_at) || (updatedOffice && updatedOffice.updated_at)
			};
	...
	function renderMySuiteBrokerOfficeApplyPage()
	{
		const content = document.getElementById("mySuiteContent");
		if (!content) return;
		content.className = "my-suite-content my-suite-content-profile";
	...
	async function saveMySuiteBrokerOfficeApply()
	{
		...
		async function openBrokerOfficeApplyFromMySuite()
		{
			...
			try {
				...
				if (!canApplyBrokerOffice) return;
				renderMySuiteBrokerOfficeApplyPage();
			} catch (err) {
...
(function () {
	...
	if (accountHoverTrigger && accountHoverDropdown) {
		...
		const closeAccountHoverDropdown = () => {
			if (typeof window.realjejuReleaseGlobalAccountDropdownFocus === "function") window.realjejuReleaseGlobalAccountDropdownFocus(accountHoverDropdown);
	...
	function pickApprovedBrokerOffice(rows)
	{
		return getVisibleBrokerOfficeRows(rows).find((row) => getBrokerOfficeRowStatus(row) === "active") || null;
	}

	function pickCurrentBrokerOffice(rows)
	{
		const visibleRows = getVisibleBrokerOfficeRows(rows);
	...
	async function updateApprovedBrokerTopbarMenu(user, profile)
	{
		...
		try {
			...
			const brokerOffice = pickApprovedBrokerOffice(officeRows);
			const isActiveOffice = !!brokerOffice;
			renderTopbarMenu(isActiveOffice);
	...
	function renderTopbarMenu(isBroker)
	{
		...
		const menus = isBroker
			? ["중개사 홈", "부동산 홈", "매물 등록", "관심매물", "공지사항"]
			: ["부동산 홈", "매물 등록", "관심매물", "공지사항"];
	...
	function renderAdminTopbarMenu()
	{
		...
		const menus = ["관리자 페이지", "부동산 홈", "매물 등록", "관심매물", "공지사항"];
		const currentMenus = Array.from(topbarMenu.querySelectorAll(".topbar-menu-item")).map((button) => button.textContent.trim());
		if (currentMenus.length === menus.length && currentMenus.every((label, index) => label === menus[index])) return;
...
function applyLoggedInAccountUI(user, profile)
	{
		...
		window.realjejuCurrentProfile = profile || null;
		window.realjejuCurrentBrokerOffice = null;
		loadFavoriteListingStateFromServer({ force: true });
	...
	async function fetchCurrentProfileAndBrokerOffice()
	{
		...
		try {
			...
			const visibleRows = rows.filter((row) => row && row.status !== "deleted" && !row.deleted_at);
			brokerOffice = visibleRows.find((row) => row && row.status === "active")
				|| visibleRows.find((row) => row && (row.owner_name || row.office_reg_no || row.office_address))
				|| visibleRows[0]
				|| null;
		} catch (err) {
	...
	async function fetchApprovedBrokerAccess()
	{
		...
		try {
			const { data: officeRows } = await client
				...
				.order("created_at", { ascending: false })
				.limit(10);
			brokerOffice = pickApprovedBrokerOffice(officeRows);
		} catch (err) {
			brokerOffice = null;
		}
		const allowed = !!(isBrokerRoleValue(profile && profile.role_request) && brokerOffice && getBrokerOfficeRowStatus(brokerOffice) === "active");
		if (allowed) {
			window.realjejuCurrentProfile = profile || null;
	...
	async function openMyInfoFromAccountMenu()
	{
		...
		try {
			...
			if (brokerBtn) {
				...
				const isPending = brokerOffice && (brokerOffice.status === "new" || brokerOffice.status === "pending");
	...
	function getRealjejuWithdrawErrorMessage(error)
	{
		...
		if (/Function not found|not found|404/i.test(message)) {
			return "delete-current-user 함수가 배포되지 않았거나 이름이 다릅니다.\nSupabase Edge Functions에서 함수 이름을 확인해 주세요.";
		}
	...
	if (profileEditPageForm) {
		async function handleProfileEditSubmit(e) {
			...
			try {
				...
				if (typeof writeRealjejuCachedProfile === "function") {
					writeRealjejuCachedProfile(user.id, {
						name: name,
						phone: phone,
						role_request: role,
						profile_completed: true,
		...
		brokerOfficeEditPageForm.addEventListener("submit", async (e) => {
			...
			if (!regNo) return openAuthErrorModal("등록번호를 입력하세요.", "중개사무소 수정", regNoInput);
				...
				let query = client
					...
					.eq("user_id", user.id);
				query = agencyId ? query.eq("id", agencyId) : query;
				const { data: updatedRows, error } = await query.select("*");
				if (error) {
					console.error("중개사무소 정보 수정 실패:", error);
					if (isRealjejuKakaoUrlColumnError(error)) return openAuthErrorModal(REALJEJU_KAKAO_URL_COLUMN_MESSAGE, "중개사무소 수정", kakaoInput || officeNameInput);
					return openAuthErrorModal("중개사무소 정보 수정에 실패했습니다.", "중개사무소 수정", null);
				}
				const updatedOffice = pickUpdatedBrokerOfficeRow(updatedRows, agencyId);
				let savedKakaoRow = null;
				try {
					savedKakaoRow = await saveRealjejuAgencyKakaoUrl(client, (updatedOffice && updatedOffice.id) || agencyId, kakaoUrl);
				} catch (kakaoError) {
					console.error("중개사무소 카카오 오픈 채팅방 저장 실패:", kakaoError);
					if (isRealjejuKakaoUrlColumnError(kakaoError) || isRealjejuMissingKakaoRpcError(kakaoError)) {
						return openAuthErrorModal("카카오 오픈 채팅방 저장 기능이 아직 DB에 설치되지 않았습니다.\nsql/admin_tools_3.084.sql을 Supabase SQL Editor에서 실행해 주세요.", "중개사무소 수정", kakaoInput || officeNameInput);
					}
					return openAuthErrorModal("카카오 오픈 채팅방 저장에 실패했습니다.", "중개사무소 수정", kakaoInput || officeNameInput);
				}
				window.realjejuCurrentBrokerOffice = {
					...(window.realjejuCurrentBrokerOffice || {}),
					...(updatedOffice || {}),
		...
		brokerOfficeApplyForm.addEventListener("submit", async (e) => {
			...
			if (!licenseNo) return openAuthErrorModal("등록번호를 입력하세요.", "중개사무소 가입 신청", document.getElementById("brokerLicenseNoInput"));
				...
				if (isEditMode) {
					...
					if (error) {
						console.error("중개사무소 정보 수정 실패:", error);
						if (isRealjejuKakaoUrlColumnError(error)) return openAuthErrorModal(REALJEJU_KAKAO_URL_COLUMN_MESSAGE, "중개사무소 정보", document.getElementById("brokerOfficeKakaoInput"));
						return openAuthErrorModal("중개사무소 정보 수정에 실패했습니다.", "중개사무소 정보", null);
					}
					const updatedOffice = pickUpdatedBrokerOfficeRow(updatedRows, brokerOfficeApplyForm.dataset.agencyId);
					let savedKakaoRow = null;
					try {
						savedKakaoRow = await saveRealjejuAgencyKakaoUrl(client, (updatedOffice && updatedOffice.id) || brokerOfficeApplyForm.dataset.agencyId, kakaoUrl);
					} catch (kakaoError) {
						console.error("중개사무소 카카오 오픈 채팅방 저장 실패:", kakaoError);
						if (isRealjejuKakaoUrlColumnError(kakaoError) || isRealjejuMissingKakaoRpcError(kakaoError)) {
							return openAuthErrorModal("카카오 오픈 채팅방 저장 기능이 아직 DB에 설치되지 않았습니다.\nsql/admin_tools_3.084.sql을 Supabase SQL Editor에서 실행해 주세요.", "중개사무소 정보", document.getElementById("brokerOfficeKakaoInput"));
						}
						return openAuthErrorModal("카카오 오픈 채팅방 저장에 실패했습니다.", "중개사무소 정보", document.getElementById("brokerOfficeKakaoInput"));
					}
					window.realjejuCurrentBrokerOffice = {
						...(window.realjejuCurrentBrokerOffice || {}),
						...(updatedOffice || {}),
						id: (savedKakaoRow && savedKakaoRow.id) || (updatedOffice && updatedOffice.id) || brokerOfficeApplyForm.dataset.agencyId,
						user_id: (updatedOffice && updatedOffice.user_id) || user.id,
						...agencyPayload,
						kakao_url: (savedKakaoRow && savedKakaoRow.kakao_url) || kakaoUrl,
						updated_at: (savedKakaoRow && savedKakaoRow.updated_at) || (updatedOffice && updatedOffice.updated_at) || agencyPayload.updated_at
					};
					cacheMapListingAgency(window.realjejuCurrentBrokerOffice);
	...
	async function openPropertyRegisterPage(options = {})
	{
		...
		const shouldKeepExistingDraft = !wasRegisterPageOpen && hasPropertyRegisterDraft();
		document.body.classList.add("property-register-page-open");
		closeAdminPage();
		closeBrokerHomePage();
	...
	function syncPropertyListingNoFromTitle()
	{
		...
		const listingNoInput = document.getElementById("propertyListingNoInput");
		if (!titleInput || !listingNoInput) return;
		const listingNo = extractPropertyListingNoFromTitle(titleInput.value);
		if (!listingNo) return;
		if (listingNoInput.value === listingNo) return;
		listingNoInput.value = listingNo;
	...
	{
		const titleInput = document.getElementById("propertyLocationSummaryInput");
		if (!titleInput || titleInput.dataset.listingNoAutoFillBound === "1") return;
	...
	{
		return "sql/admin_tools_3.084.sql을 Supabase SQL Editor에서 실행한 뒤 다시 시도하세요.";
	}
	...
	function isAdminInquiryDeleted(row)
	{
		...
		return status === "deleted" || !!(row && row.deleted_at);
	}
	...
	function isAdminInquiryAnswered(row)
	{
		...
		const status = String(row && row.status || "").trim().toLowerCase();
		return status === "answered" || !!String(row && row.admin_reply || "").trim();
	}
	...
	async function loadAdminInquiries()
	{
		...
		try {
			...
			updateAdminInquiryFilterCounts(inquiries);
			const filteredRows = getAdminInquiryFilteredRows(inquiries);
			if (!inquiries.length) {
			...
			listEl.innerHTML = `
				<div class="admin-inquiry-table">
					${filteredRows.map((row) => {
						...
						const author = String(row.author_label || row.author_name || row.author_email || "").trim() || "작성자 미확인";
						const status = deleted ? "휴지통" : (answered ? "답변완료" : "미답변");
						return `
							<article class="admin-inquiry-item ${deleted ? "deleted" : (answered ? "answered" : "pending")}">
								<div class="admin-inquiry-row">
									<div class="admin-inquiry-date">${escapeAdminHtml(formatAdminInquiryDate(row.created_at))}</div>
									<div class="admin-inquiry-main">
										<div class="admin-inquiry-title">${escapeAdminHtml(row.title || "제목 없음")}</div>
										<div class="admin-inquiry-author">${escapeAdminHtml(author)}</div>
									</div>
	...
	function isAdminProfileRow(row)
	{
		...
		return role === "admin" || email === "test01@naver.com";
	}
	...
	{
		const list = Array.isArray(rows) ? rows : [];
		if (adminUserView === "broker") return list.filter((row) => isBrokerRoleValue(row && row.role_request));
		if (adminUserView === "incomplete") return list.filter(isAdminUserIncomplete);
		if (adminUserView === "admin") return list.filter(isAdminProfileRow);
		return list;
	}
	...
	function updateAdminUserFilterCounts(rows)
	{
		...
		setText("adminUsersAllCount", list.length);
		setText("adminUsersBrokerCount", list.filter((row) => isBrokerRoleValue(row && row.role_request)).length);
		setText("adminUsersIncompleteCount", list.filter(isAdminUserIncomplete).length);
		setText("adminUsersAdminCount", list.filter(isAdminProfileRow).length);
	...
	function renderAdminUsers(rows)
	{
		...
		` + filteredRows.map((row) => {
			...
			const roleLabel = isAdminProfileRow(row) ? "관리자" : getRoleLabel(row.role_request || "user");
			const passLabel = getAdminUserPassLabel(row);
			const profileState = isAdminProfileRow(row) ? "관리자" : (isAdminUserIncomplete(row) ? "미완료" : "완료");
			const stateClass = isAdminProfileRow(row) ? "admin" : (isAdminUserIncomplete(row) ? "incomplete" : "published");
			return `
	...
	function openAdminUserDetail(userId, returnFocusTarget)
	{
		...
		if (!row) return;
		const lines = [
			`이름 : ${String(row.name || "이름 없음").trim()}`,
			`이메일 : ${String(row.email || "-").trim()}`,
			`연락처 : ${formatDisplayPhone(row.phone) || "-"}`,
			`회원유형 : ${isAdminProfileRow(row) ? "관리자" : getRoleLabel(row.role_request || "user")}`,
			`이용권 : ${getAdminUserPassLabel(row)}`,
			`상태 : ${isAdminProfileRow(row) ? "관리자" : (isAdminUserIncomplete(row) ? "미완료" : "완료")}`,
	...
	{
		const list = Array.isArray(rows) ? rows : [];
		if (adminListingView === "reported") return list.filter(isAdminListingReported);
		if (adminListingView === "hidden") return list.filter((row) => ["hidden", "archive"].includes(getBrokerListingEffectiveStatus(row)));
		if (adminListingView === "deleted") return list.filter((row) => getBrokerListingEffectiveStatus(row) === "deleted");
		return list;
	}
	...
	function updateAdminListingFilterCounts(rows)
	{
		...
		setText("adminListingsAllCount", list.length);
		setText("adminListingsReportedCount", list.filter(isAdminListingReported).length);
		setText("adminListingsHiddenCount", list.filter((row) => ["hidden", "archive"].includes(getBrokerListingEffectiveStatus(row))).length);
		setText("adminListingsDeletedCount", list.filter((row) => getBrokerListingEffectiveStatus(row) === "deleted").length);
				...
				<div class="broker-listing-row admin-listing-row" data-admin-listing-id="${rowId}">
					...
					<div class="broker-listing-menu-cell">
						...
						<div class="broker-listing-more-menu" data-admin-listing-menu-panel="${rowId}">
							<button type="button" class="broker-listing-menu-item" data-admin-listing-action="view" data-listing-id="${rowId}">상세 보기</button>
						</div>
					</div>
				</div>
			`;
	...
	async function loadAdminListings(options = {})
	{
		...
		adminListingLoadPromise = (async () => {
			const { data, error } = await client
				.from("property_listings")
				.select("id, user_id, status, property_type, property_type_label, deal_types, title, public_address, created_at, updated_at, photos, payload")
				.order("updated_at", { ascending: false });

			if (error) {
				console.error("관리자 매물 목록 조회 실패:", error);
				listEl.innerHTML = '<div class="admin-empty">매물을 불러오지 못했습니다.</div>';
				return;
			}
	...
	function fillBrokerListingEditForm(row)
	{
		...
		setBrokerEditInput("propertyListingNoInput", row.listing_no || payload.listing_no || getBrokerListingNo(row));
	...
	async function updateAdminApplicationStatus(agencyId, status)
	{
		...
		try {
			...
			if (status === "deleted") {
				const { data: agencyRow, error: agencyFetchError } = await client
					.from("agencies")
					.select("user_id")
					.eq("id", agencyId)
					.maybeSingle();
				if (agencyFetchError) {
					console.error("중개사무소 삭제 대상 조회 실패:", agencyFetchError);
					openAuthErrorModal("삭제 대상 확인에 실패했습니다.", "관리자 페이지", null);
					return;
				}
				query = agencyRow && agencyRow.user_id
					? query.eq("user_id", agencyRow.user_id)
					: query.eq("id", agencyId);
			} else {
	...
	document.addEventListener("click", function(e) {
		...
		if (adminInquiryViewBtn) {
			...
			if (Array.isArray(adminInquiryRowsCache)) {
				updateAdminInquiryFilterCounts(adminInquiryRowsCache);
				const listEl = document.getElementById("adminInquiryList");
				if (listEl) {
					const rows = adminInquiryRowsCache;
					const filteredRows = getAdminInquiryFilteredRows(rows);
					if (!filteredRows.length) listEl.innerHTML = '<div class="admin-empty">해당 상태의 1:1 문의가 없습니다.</div>';
		...
		if (adminUserActionBtn) {
			...
			if (adminUserActionBtn.dataset.adminUserAction === "view") {
		...
		try {
			...
			if (showBrokerOfficeMenu) {
				try {
					...
					latestBrokerOfficeName = Array.isArray(agencyRows) && agencyRows[0] ? String(agencyRows[0].office_name || "").trim() : "";
...
(function bindPropertyListingSave()
{
	...
	function collectListingPayload(status)
	{
		...
		const propertyTypeValue = syncRegisterDropdownSelectValue(propertyTypeSelect);
		...
		const listingNo = (typeof extractPropertyListingNoFromTitle === "function" ? extractPropertyListingNoFromTitle(title) : "") || value("propertyListingNoInput");
	...
	function hasRequiredControlValue(control)
	{
		...
		if (control.matches("select.property-register-native-select")) {
			syncRegisterDropdownSelectValue(control);
		}
	...
	function hasRequiredFieldValue(row, label)
	{
		...
		controls.forEach((control) => {
			if (control.matches("select.property-register-native-select")) syncRegisterDropdownSelectValue(control);
		});
...
(function bindAuthStateGoHome()
{
	...
	function goRealjejuHome()
	{
		...
		document.body.classList.remove("auth-page-open", "property-register-page-open", "admin-page-open", "broker-home-page-open", "payment-page-open", "myinfo-page-open", "profile-edit-page-open", "broker-office-info-page-open", "broker-office-edit-page-open", "profile-page-open", "my-suite-page-open", "notice-page-open");
