...
function findRealjejuPropertyTypeOption(select, ...values)
{
	...
	for (const raw of values) {
		...
		const alias = REALJEJU_PROPERTY_TYPE_ALIAS_MAP[normalizeRealjejuPropertyTypeKey(text)];
		if (alias) {
...
function syncRealjejuPropertyTypeSelect(select, ...values)
{
	...
	if (!option) return String(select.value || "").trim();
...
function getBrokerOfficeRowStatus(row)
{
	...
	if (["new", "applied", "apply", "신청", "가입신청", "신청중"].includes(compact)) return "new";
	if (["pending", "waiting", "wait", "대기", "승인대기", "대기중"].includes(compact)) return "pending";
...
async function fetchSharedBrokerOfficeRows(client, user, profile)
{
	...
	const promise = (async () => {
		...
		const rows = mergeBrokerOfficeRows(userRows, emailRows);
...
{
	const resetPagination = options && options.resetPagination !== false;
	...
	if (resetPagination) resetLeftListPagination();
	const listItems = getPagedLeftListItems(viewportItems);
	...
	setPagedListInfo(viewportItems.length, listItems.length);
	if (listItems.length) {
		state.lastViewportListIds = listItems.map(item => normalizeItemId(item.id));
	}
	refreshRecommendAgentCard();
	refreshRecommendationCard(state.filtered);
	return listItems;
...
async function runLoadProperties()
{
	try {
		...
		const normalized = rows
			...
			.map(row => mapPropertyListingRowToMapItem(row))
...
function resetLeftListPagination()
{
	state.leftListVisibleCount = LEFT_LIST_PAGE_SIZE;
	state.leftListRenderedCount = 0;
	state.leftListTotalCount = 0;
	setLeftListLoading(false);
}

function getPagedLeftListItems(items)
{
	const list = Array.isArray(items) ? items : [];
	const visibleCount = Math.max(LEFT_LIST_PAGE_SIZE, Number(state.leftListVisibleCount || LEFT_LIST_PAGE_SIZE));
	state.leftListTotalCount = list.length;
	state.leftListRenderedCount = Math.min(visibleCount, list.length);
	return list.slice(0, state.leftListRenderedCount);
}
...
function loadMoreLeftListItems()
{
	if (state.lockedListIds || document.body.classList.contains("sidebar-list-collapsed")) return;
	...
	setTimeout(() => {
		state.leftListVisibleCount = Math.min(total, rendered + LEFT_LIST_PAGE_SIZE);
		setLeftListLoading(false);
		refreshViewportList({ resetPagination: false });
	}, 120);
}
...
function renderList(data)
{
	...
	const hasMoreListItems = !state.lockedListIds && Number(state.leftListRenderedCount || 0) < Number(state.leftListTotalCount || 0);
...
function applyFilter()
{
	...
	const listItems = shouldHideInitialList ? [] : getPagedLeftListItems(sortedViewportItems);
	scrollListToTop();
	...
	if (shouldHideInitialList) {
		...
		setPagedListInfo(sortedViewportItems.length, listItems.length);
	}
		...
		try {
			...
			try {
				...
				if (isRealjejuKakaoUrlColumnError(kakaoError) || isRealjejuMissingKakaoRpcError(kakaoError)) {
					return openAuthErrorModal("카카오 오픈 채팅방 저장 기능이 아직 DB에 설치되지 않았습니다.\nsql/admin_tools_3.130.sql을 Supabase SQL Editor에서 실행해 주세요.", "중개사 정보", kakaoInput || officeNameInput);
				}
...
(function () {
	...
	async function updateApprovedBrokerTopbarMenu(user, profile)
	{
		...
		try {
			...
			const brokerOffice = pickApprovedBrokerOffice(officeRows)
				|| (isApprovedBrokerOffice(window.realjejuCurrentBrokerOffice) ? window.realjejuCurrentBrokerOffice : null);
			...
			const cachedOffice = isApprovedBrokerOffice(window.realjejuCurrentBrokerOffice) ? window.realjejuCurrentBrokerOffice : null;
			renderTopbarMenu(!!cachedOffice);
			if (cachedOffice && cachedOffice.office_name) setTopbarAccountDisplayName(cachedOffice.office_name);
		}
	}
	...
	async function fetchMyLatestBrokerOfficeStatus(user)
	{
		...
		try {
			...
			const row = pickCurrentBrokerOffice(rows);
	...
	async function updateBrokerOfficeDropdownMenu(user, profile)
	{
		...
		if (!isBroker && !cachedApprovedOffice) {
...
function applyLoggedInAccountUI(user, profile)
	...
	async function fetchCurrentProfileAndBrokerOffice()
	{
		...
		try {
			...
			brokerOffice = visibleRows.find((row) => row && getBrokerOfficeRowStatus(row) === "active")
				|| visibleRows.find((row) => row && (row.owner_name || row.office_reg_no || row.office_address))
				|| visibleRows[0]
				|| null;
		} catch (err) {
	...
	async function fetchApprovedBrokerAccess()
	{
		...
		if (!brokerOffice && isApprovedBrokerOffice(window.realjejuCurrentBrokerOffice)) {
			...
			if (!cachedOffice.user_id || String(cachedOffice.user_id) === String(user.id)) brokerOffice = cachedOffice;
		}
		const allowed = isApprovedBrokerOffice(brokerOffice);
		if (allowed) {
			window.realjejuCurrentProfile = profile || null;
			window.realjejuCurrentBrokerOffice = brokerOffice;
			renderTopbarMenu(true);
		} else {
			window.realjejuCurrentBrokerOffice = null;
			renderTopbarMenu(false);
		}
	...
	async function openMyInfoFromAccountMenu()
	{
		...
		try {
			...
			if (isAdminUser(user)) {
				...
				renderTopbarMenu(isApprovedBrokerOffice(brokerOffice));
			}
			...
			if (brokerBtn) {
				...
				brokerBtn.style.display = (brokerRole || isActive) ? "block" : "none";
		...
		brokerOfficeEditPageForm.addEventListener("submit", async (e) => {
			...
			if (!regNo) return openAuthErrorModal("등록번호를 입력하세요.", "중개사무소 수정", regNoInput);
				...
				try {
					...
					if (isRealjejuKakaoUrlColumnError(kakaoError) || isRealjejuMissingKakaoRpcError(kakaoError)) {
						return openAuthErrorModal("카카오 오픈 채팅방 저장 기능이 아직 DB에 설치되지 않았습니다.\nsql/admin_tools_3.130.sql을 Supabase SQL Editor에서 실행해 주세요.", "중개사무소 수정", kakaoInput || officeNameInput);
					}
			...
			if (!licenseNo) return openAuthErrorModal("등록번호를 입력하세요.", "중개사무소 가입 신청", document.getElementById("brokerLicenseNoInput"));
				...
				if (isEditMode) {
					...
					try {
						...
						console.error("중개사무소 카카오 오픈 채팅방 저장 실패:", kakaoError);
						if (isRealjejuKakaoUrlColumnError(kakaoError) || isRealjejuMissingKakaoRpcError(kakaoError)) {
							return openAuthErrorModal("카카오 오픈 채팅방 저장 기능이 아직 DB에 설치되지 않았습니다.\nsql/admin_tools_3.130.sql을 Supabase SQL Editor에서 실행해 주세요.", "중개사무소 정보", document.getElementById("brokerOfficeKakaoInput"));
						}
	...
	{
		return "sql/admin_tools_3.130.sql을 Supabase SQL Editor에서 실행한 뒤 다시 시도하세요.";
	}
	...
	function getAdminUserFilteredRows(rows)
	{
		...
		if (adminUserView === "broker") return visibleRows.filter((row) => isBrokerRoleValue(row && row.role_request));
	...
	function updateAdminUserFilterCounts(rows)
	{
		...
		setText("adminUsersBrokerCount", visibleRows.filter((row) => isBrokerRoleValue(row && row.role_request)).length);
	...
	async function updateAdminApplicationStatus(agencyId, status)
	{
		...
		try {
			const nowIso = new Date().toISOString();
			const updateValues = { status, updated_at: nowIso };
			if (status === "deleted") updateValues.deleted_at = nowIso;
			else updateValues.deleted_at = null;
			let query = client
				.from("agencies")
				.update(updateValues);
			if (status === "deleted") {
