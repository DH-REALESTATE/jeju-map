...
function normalizeRealjejuPhone(value)
	...
	async function fetchMySuiteProfileAndBrokerOffice()
	{
		...
		try {
			...
			brokerOffice = rows.find((row) => row && row.status === "active")
				|| rows.find((row) => row && (row.owner_name || row.office_reg_no || row.office_address))
				|| rows[0]
				|| null;
		} catch (err) {
			brokerOffice = null;
	...
	async function renderMySuiteProfileContent()
	{
		...
		function renderProfileCard(user, profile, brokerOffice)
		{
			...
			const brokerActionLabel = brokerOffice && brokerOffice.status === "active"
				? "중개사 정보"
				: (brokerOffice && brokerOffice.status === "pending" ? "가입 신청 진행중" : "중개사무소 가입 신청");
			const brokerAction = brokerOffice && brokerOffice.status === "active"
				? "broker-office-info"
				: (brokerOffice && brokerOffice.status === "pending" ? "broker-office-pending" : "broker-office-apply");
		...
		const fetchDetailedBrokerOffice = async (user, currentOffice) => {
			...
			try {
				...
				const detailed = (currentId ? list.find((row) => row && String(row.id || "") === currentId) : null)
					...
					|| list[0]
					|| null;
				if (!detailed) return currentOffice || null;
				const mergedOffice = {
					...(currentOffice || {}),
					...detailed
				};
...
(function () {
	...
	async function updateApprovedBrokerTopbarMenu(user, profile)
	{
		...
		try {
			...
			const isActiveOffice = !!(brokerOffice && brokerOffice.status === "active");
	...
	async function fetchApprovedBrokerAccess()
	{
		...
		try {
			const { data: officeRows } = await client
				...
				.limit(10);
			const rows = Array.isArray(officeRows) ? officeRows : [];
			brokerOffice = rows.find(row => row && row.status === "active") || rows[0] || null;
		} catch (err) {
			brokerOffice = null;
		}
		const allowed = !!(isBrokerRoleValue(profile && profile.role_request) && brokerOffice && brokerOffice.status === "active");
		if (allowed) {
			window.realjejuCurrentProfile = profile || null;
...
(function bindPropertyListingSave()
{
	...
	function syncRegisterDropdownSelectValue(select)
	{
		...
		const wrap = select.closest("[data-register-dropdown]");
		if (!wrap) return "";
		const activeOption = wrap.querySelector(".property-register-dropdown-option.active[data-value]");
		const labelText = String(wrap.querySelector("[data-register-dropdown-label]")?.textContent || "").trim();
		const option = activeOption || Array.from(wrap.querySelectorAll(".property-register-dropdown-option[data-value]")).find((btn) => {
			return String(btn.textContent || "").trim() === labelText;
		});
