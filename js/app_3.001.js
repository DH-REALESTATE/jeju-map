...
function applyLoggedInAccountUI(user, profile)
	...
	async function openAdminPage(tabKey = "notices")
	{
	...
	async function loadAdminInquiries()
	{
		...
		try {
			...
			const userIds = [...new Set(inquiries.map((row) => String(row.user_id || "").trim()).filter(Boolean))];
			if (userIds.length) {
				try {
					...
					(Array.isArray(profileRows) ? profileRows : []).forEach((profile) => {
						...
						if (key && label) authorMap.set(key, label);
					});
				} catch (authorError) {
					console.warn("관리자 1:1 문의 작성자 조회 실패:", authorError);
				}
			}
			if (!inquiries.length) {
			...
			listEl.innerHTML = `
				<div class="admin-inquiry-table">
					${inquiries.map((row) => {
						const id = escapeAdminHtml(row.id || "");
						const answered = !!String(row.admin_reply || "").trim();
						const author = String(row.author_name || row.author_email || "").trim() || authorMap.get(String(row.user_id || "").trim()) || "작성자 미확인";
						const status = answered ? "답변완료" : "미답변";
						return `
							<article class="admin-inquiry-item ${answered ? "answered" : "pending"}">
								<div class="admin-inquiry-row">
									<div class="admin-inquiry-date">${escapeAdminHtml(formatMySuiteInquiryDate(row.created_at))}</div>
			...
			listEl.innerHTML = '<div class="admin-empty">1:1 문의내역을 불러오지 못했습니다.</div>';
		}
	}
				...
				return `
					<div class="admin-application-card" data-agency-id="${escapeAdminHtml(row.id)}">
						...
						<div class="admin-application-meta">
							...
							<div>연락처: ${escapeAdminHtml(row.phone || "-")}</div>
							<div>이메일: ${escapeAdminHtml(row.email || "-")}</div>
						</div>
						<div>
	...
	document.addEventListener("click", function(e) {
		...
		if (adminTabBtn) {
			...
			if (tab === "broker-applications") loadAdminApplications();
			if (tab === "inquiries") loadAdminInquiries();
			return;
		}
