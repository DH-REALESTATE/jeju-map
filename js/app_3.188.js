		...
		if (error) throw error;
			...
			const normalized = rows
				.filter(row => {
					...
					const effectiveStatus = String(payload.broker_status || "").trim().toLowerCase();
	...
	async function openBrokerHomePage(currentListingId)
	{
		...
		async function updateAdminApplicationStatus(agencyId, status)
		{
			...
			try {
				...
				adminApplicationView = status === "active" ? "active" : (status === "pending" ? "pending" : (status === "rejected" ? "rejected" : (status === "deleted" ? "deleted" : "new")));
