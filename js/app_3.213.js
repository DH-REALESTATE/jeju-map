...
function applyLoggedInAccountUI(user, profile)
	...
	async function openAdminPage(tabKey = null)
	{
		...
		if (adminContext.allowed) renderAdminTopbarMenu(isApprovedBrokerOffice(window.realjejuCurrentBrokerOffice));
	...
	function formatAdminUserLastLoginDate(row)
	{
		...
		function isAdminUserApprovedBroker(row)
		{
			...
			if (!isBrokerRoleValue(row.role_request)) return false;
			return String(row.approved_agency_status || "").trim() === "active";
		}

		function getAdminUserRoleLabel(row)
		{
			if (isAdminProfileRow(row)) return "관리자";
			return isAdminUserApprovedBroker(row) ? getRoleLabel(row.role_request || "user") : "일반회원";
		}
