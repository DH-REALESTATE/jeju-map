...
let viewMap = {};
const VIEW_COUNT_RPC_ENABLED = false;
...
function refreshMapLayout()
{
	...
	const run = () => {
		try {
			...
			if (currentDetailItem && window.kakao && kakao.maps && Number.isFinite(Number(currentDetailItem.lat)) && Number.isFinite(Number(currentDetailItem.lng))) {
				state.map.setCenter(new kakao.maps.LatLng(Number(currentDetailItem.lat), Number(currentDetailItem.lng)));
			}
		} catch (error) {
...
async function loadMapListingOwnerData(client, rows)
{
	...
	if (agencyIds.length) {
		try {
			...
			(Array.isArray(data) ? data : []).forEach(agency => {
				...
				if (agency.office_name) agenciesByOfficeName.set(String(agency.office_name || "").trim(), preferAgency(agenciesByOfficeName.get(String(agency.office_name || "").trim()), agency));
				if (agency.user_id) {
					const key = String(agency.user_id);
					agenciesByUserId.set(key, preferAgency(agenciesByUserId.get(key), agency));
				}
			});
		} catch (err) {
	...
	if (userIds.length) {
		try {
			...
			(Array.isArray(data) ? data : []).forEach(agency => {
				const key = String(agency.user_id || "");
				if (key) agenciesByUserId.set(key, preferAgency(agenciesByUserId.get(key), agency));
				if (agency.office_name) agenciesByOfficeName.set(String(agency.office_name || "").trim(), preferAgency(agenciesByOfficeName.get(String(agency.office_name || "").trim()), agency));
				if (agency.id) {
					const idKey = String(agency.id);
					agenciesById.set(idKey, preferAgency(agenciesById.get(idKey), agency));
				}
			});
		} catch (err) {
	...
	if (officeNames.length) {
		try {
			const { data, error } = await client
				.from("agencies")
				.select("*")
				.in("office_name", officeNames);
			if (error) throw error;
			(Array.isArray(data) ? data : []).forEach(agency => {
				const officeKey = String(agency.office_name || "").trim();
				if (officeKey) agenciesByOfficeName.set(officeKey, preferAgency(agenciesByOfficeName.get(officeKey), agency));
				if (agency.id) {
					const idKey = String(agency.id);
					agenciesById.set(idKey, preferAgency(agenciesById.get(idKey), agency));
				}
				if (agency.user_id) {
					const userKey = String(agency.user_id);
					agenciesByUserId.set(userKey, preferAgency(agenciesByUserId.get(userKey), agency));
				}
			});
		} catch (err) {
...
async function resolveDetailData(item)
{
	...
	return {
		...
		linkKakaotalk: firstText(dbItem.linkKakaotalk)
	};
}
...
async function openDetailPanel(item, options = {})
{
	...
	if (detailSummaryAddress) {
		detailSummaryAddress.textContent = formatDisplayPropertyAddress(detailItem.address);
	}
	...
	if (detailItem.agentName || detailItem.agentOffice || detailItem.phone1 || detailItem.phone2 || detailItem.agentImage) {
		...
		detailAgentAddress.textContent = detailItem.agentAddress ? `주소 ${detailItem.agentAddress}` : "";
	...
	const detailKakaoUrl = String(detailItem.linkKakaotalk || "").trim();
...
function normalizeRealjejuPhone(value)
	...
	if (detailCopyLinkBtn) {
		detailCopyLinkBtn.addEventListener("click", async (e) => {
			...
			try {
				...
				closeShareMenu();
				setTimeout(() => {
					detailShareBtn.innerHTML = '<i class="fa-solid fa-arrow-up-from-bracket"></i>';
				}, 1000);
			} catch (err) {
				console.error(err);
			}
	...
	async function renderMySuiteInquiriesContent()
	{
		...
		try {
			...
			let isAdminInquiryViewer = false;
			try {
				const { data: profileData } = await client
					.from("profiles")
					.select("role")
					.eq("id", user.id)
					.maybeSingle();
				isAdminInquiryViewer = String(profileData && profileData.role || "").trim() === "admin";
			} catch (profileError) {
				console.warn("1:1 문의 관리자 권한 확인 실패:", profileError);
			}
			let inquiryQuery = client
				...
				.order("created_at", { ascending: false });
			if (!isAdminInquiryViewer) inquiryQuery = inquiryQuery.eq("user_id", user.id);
			const { data, error } = await inquiryQuery;
			if (error) throw error;
			const inquiries = Array.isArray(data) ? data : [];
			const inquiryAuthorMap = new Map();
			if (isAdminInquiryViewer && inquiries.length) {
				const inquiryUserIds = [...new Set(inquiries.map((row) => String(row.user_id || "").trim()).filter(Boolean))];
				if (inquiryUserIds.length) {
					try {
						const { data: profileRows, error: profileListError } = await client
							.from("profiles")
							.select("id, name, email")
							.in("id", inquiryUserIds);
						if (profileListError) throw profileListError;
						(Array.isArray(profileRows) ? profileRows : []).forEach((profile) => {
							const key = String(profile && profile.id || "").trim();
							if (!key) return;
							const label = String(profile && (profile.name || profile.email) || "").trim();
							if (label) inquiryAuthorMap.set(key, label);
						});
					} catch (authorError) {
						console.warn("1:1 문의 작성자 정보 조회 실패:", authorError);
					}
				}
			}
			content.innerHTML = `
				<div class="my-suite-inquiry-head">
					<div>
						...
						<p class="my-suite-content-desc" id="mySuiteContentDesc">${isAdminInquiryViewer ? "전체 1:1 문의를 확인할 수 있습니다." : "문의하신 내용을 확인하고 새 문의를 남길 수 있습니다."}</p>
					</div>
				</div>
							...
							<article class="my-suite-inquiry-item" data-inquiry-id="${id}">
								<div class="my-suite-inquiry-row">
									<button type="button" class="my-suite-inquiry-main" data-my-suite-inquiry-toggle="${id}">
										...
										<span class="my-suite-inquiry-title">${isAdminInquiryViewer ? escapeMySuiteHtml((String(row.author_name || row.author_email || "").trim() || inquiryAuthorMap.get(String(row.user_id || "").trim()) || "작성자 미확인") + " · ") : ""}${escapeMySuiteHtml(row.title || "제목 없음")}</span>
										<span class="my-suite-inquiry-state ${answered ? "answered" : "pending"}">${status}</span>
									</button>
									${isAdminInquiryViewer ? "" : `<button type="button" class="my-suite-inquiry-delete" data-my-suite-inquiry-delete="${id}">삭제</button>`}
								</div>
								<div class="my-suite-inquiry-detail">
									...
									<div class="my-suite-inquiry-detail-block answer">
										...
										${isAdminInquiryViewer ? `
										<form class="my-suite-inquiry-reply-form" data-my-suite-inquiry-reply-form="${id}">
											<label class="my-suite-inquiry-label" for="mySuiteInquiryReplyInput-${id}">답변 작성</label>
											<textarea class="profile-suite-input my-suite-inquiry-reply-textarea" id="mySuiteInquiryReplyInput-${id}" maxlength="1000" required>${answered ? escapeMySuiteHtml(row.admin_reply) : ""}</textarea>
	...
	async function submitMySuiteInquiryReply(form)
	{
		...
		try {
			...
			if (String(profileData && profileData.role || "").trim() !== "admin") throw new Error("관리자 권한이 필요합니다.");
			const { error } = await client
				.from("support_inquiries")
				.update({
					admin_reply: reply,
			...
			if (statusEl) statusEl.textContent = "답변 등록에 실패했습니다.";
		} finally {
			form.dataset.saving = "";
		}
	}
	...
	async function renderMySuiteBrokerOfficeContent()
	{
		...
		const normalizeBrokerOffice = (office) => {
			...
			return {
				...
				phone: firstValue(data.phone, data.office_phone, data.tel),
	...
	function openMySuitePage(tabKey)
	{
		...
		document.body.classList.remove("broker-home-page-open", "admin-page-open", "property-register-page-open", "payment-page-open", "myinfo-page-open", "profile-edit-page-open", "broker-office-info-page-open", "broker-office-edit-page-open", "profile-page-open");
		...
		["brokerHomePanel", "adminPagePanel", "propertyRegisterPage", "paymentPagePanel", "myInfoPagePanel", "profileEditPagePanel", "brokerOfficeInfoPagePanel", "brokerOfficeEditPagePanel"].forEach((id) => {
	...
	async function saveMySuiteProfileEdit()
	{
		...
		const phone = String(phoneInput && phoneInput.value || "").replace(/[^0-9]/g, "");
	...
	function renderMySuiteInlineBrokerOfficeEditPage(brokerOffice)
	{
		...
		const office = {
			...
			phone: firstValue(brokerOffice && brokerOffice.phone, brokerOffice && brokerOffice.office_phone, brokerOffice && brokerOffice.tel),
	...
	async function saveMySuiteBrokerOfficeEdit()
	{
		...
		const phone = String(phoneInput && phoneInput.value || "").replace(/\D/g, "");
		...
		try {
			...
			const updatedOffice = Array.isArray(updatedRows) && updatedRows[0] ? updatedRows[0] : null;
			window.realjejuCurrentBrokerOffice = {
				...(window.realjejuCurrentBrokerOffice || {}),
				...(updatedOffice || {}),
				id: agencyId || (window.realjejuCurrentBrokerOffice && window.realjejuCurrentBrokerOffice.id) || "",
				office_name: officeName,
				owner_name: owner,
				office_reg_no: regNo,
				office_address: address,
				...
				email: email,
				kakao_url: kakaoUrl,
				status: (window.realjejuCurrentBrokerOffice && window.realjejuCurrentBrokerOffice.status) || "active"
			};
			renderMySuiteBrokerOfficeContent();
		} catch (err) {
...
(function () {
	...
	function openPaymentPageFromAccountMenu()
	{
		...
		["brokerHomePanel", "adminPagePanel", "propertyRegisterPage"].forEach((id) => {
			const el = document.getElementById(id);
			if (el) {
				if (id === "brokerHomePanel" && typeof window.realjejuReleaseFocusBeforeAriaHidden === "function") window.realjejuReleaseFocusBeforeAriaHidden(el);
				el.setAttribute("aria-hidden", "true");
...
function applyLoggedInAccountUI(user, profile)
	...
	function fillBrokerOfficeInfoScreen(brokerOffice)
	{
		...
		setTextById("brokerOfficeInfoKakaoValue", brokerOffice && brokerOffice.kakao_url);
	}
	...
	function fillBrokerOfficeApplyForm(brokerOffice)
	{
		...
		setValueById("brokerOfficeKakaoInput", brokerOffice.kakao_url);
	}
	...
	function fillBrokerOfficePageInfo(brokerOffice)
	{
		...
		setTextById("brokerOfficePagePhoneValue", brokerOffice && brokerOffice.phone);
		setTextById("brokerOfficePageEmailValue", brokerOffice && brokerOffice.email);
		setTextById("brokerOfficePageKakaoValue", brokerOffice && brokerOffice.kakao_url);
	...
	function fillBrokerOfficeEditPage(brokerOffice)
	{
		...
		setValueById("brokerOfficeEditPagePhoneInput", brokerOffice && brokerOffice.phone);
		setValueById("brokerOfficeEditPageEmailInput", brokerOffice && brokerOffice.email);
		setValueById("brokerOfficeEditPageKakaoInput", brokerOffice && brokerOffice.kakao_url);
	}
	...
	async function openBrokerOfficeInfoFromAccountMenu()
	{
		...
		try {
			...
			fillBrokerOfficePageInfo(brokerOffice);
...
(function bindAuthStateGoHome()
{
	...
	function goRealjejuHome()
	{
		...
		document.body.classList.remove("auth-page-open", "property-register-page-open", "admin-page-open", "broker-home-page-open", "payment-page-open", "myinfo-page-open", "profile-edit-page-open", "broker-office-info-page-open", "broker-office-edit-page-open", "profile-page-open", "my-suite-page-open");
		...
		["myInfoPagePanel", "profileEditPagePanel", "brokerOfficeInfoPagePanel", "brokerOfficeEditPagePanel", "paymentPagePanel", "mySuitePanel"].forEach((id) => {
			const panel = document.getElementById(id);
			if (panel) panel.setAttribute("aria-hidden", "true");
		});
