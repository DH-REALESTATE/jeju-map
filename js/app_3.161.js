...
function formatDetailAreaM2(value)
{
	const num = parseDetailAreaNumber(value);
	return Number.isFinite(num) ? formatSingleAreaValue(num, "m2") : "";
}
...
function getMapListingSections(row)
{
	...
	if (isLandListing) {
		const landAreaValue = formatDetailAreaM2(areas.land_m2 || areas.supply_m2 || areas.exclusive_m2);
		return [
			{
				title: "기본 정보",
				rows: [
					...
					{ label: "대지 면적", value: landAreaValue }
				]
			},
			{
				title: "매물 정보",
	...
	const sections = [
		{
			...
			rows: [
				...
				{ label: "전용 면적", value: formatDetailAreaM2(areas.exclusive_m2) },
				{ label: "공급 면적", value: formatDetailAreaM2(areas.supply_m2) },
				{ label: "대지 면적", value: formatDetailAreaM2(areas.land_m2) }
			]
		},
		{
			title: "매물 정보",
...
function closeManagedListingMenus()
{
	...
	document.querySelectorAll("[data-admin-listing-menu-panel].open, [data-admin-user-menu-panel].open").forEach((menu) => {
		...
		try {
			...
			try {
				...
				if (isRealjejuKakaoUrlColumnError(error) || isRealjejuMissingKakaoRpcError(error)) {
					return openAuthErrorModal("중개사 정보 저장 기능이 아직 DB에 설치되지 않았습니다.\nsql/admin_tools_3.153.sql을 Supabase SQL Editor에서 실행해 주세요.", "중개사 정보", kakaoInput || officeNameInput);
				}
...
(function () {
	...
	async function updateApprovedBrokerTopbarMenu(user, profile)
	{
		...
		try {
			...
			renderTopbarMenu(isActiveOffice, isAdminUser(user));
			...
			renderTopbarMenu(!!cachedOffice, isAdminUser(user));
	...
	function renderTopbarMenu(isBroker, isAdmin = false)
	{
	...
	function isAdminUser(user, profile = window.realjejuCurrentProfile)
	{
		...
		return !!(window.realjejuCurrentIsAdmin || isAdminProfileValue(profile) || metadataRole === "admin");
	}
...
function applyLoggedInAccountUI(user, profile)
	...
	async function fetchApprovedBrokerAccess()
	{
		...
		if (allowed) {
			...
			renderTopbarMenu(true, isAdminUser(user));
		} else {
			window.realjejuCurrentBrokerOffice = null;
			renderTopbarMenu(false, isAdminUser(user));
		}
	...
	async function openMyInfoFromAccountMenu()
	{
		...
		try {
			...
			const completed = !!(profile && profile.profile_completed === true && profile.name && profile.phone);
			if (isAdminUser(user)) {
				renderAdminTopbarMenu(isApprovedBrokerOffice(brokerOffice));
			} else {
		...
		brokerOfficeEditPageForm.addEventListener("submit", async (e) => {
			...
			if (!regNo) return openAuthErrorModal("등록번호를 입력하세요.", "중개사무소 수정", regNoInput);
				...
				try {
					...
					if (isRealjejuKakaoUrlColumnError(error) || isRealjejuMissingKakaoRpcError(error)) {
						return openAuthErrorModal("중개사 정보 저장 기능이 아직 DB에 설치되지 않았습니다.\nsql/admin_tools_3.153.sql을 Supabase SQL Editor에서 실행해 주세요.", "중개사무소 수정", kakaoInput || officeNameInput);
					}
			...
			if (!licenseNo) return openAuthErrorModal("등록번호를 입력하세요.", "중개사무소 가입 신청", document.getElementById("brokerLicenseNoInput"));
				...
				if (isEditMode) {
					...
					try {
						...
						console.error("중개사무소 정보 수정 실패:", error);
						if (isRealjejuKakaoUrlColumnError(error) || isRealjejuMissingKakaoRpcError(error)) {
							return openAuthErrorModal("중개사 정보 저장 기능이 아직 DB에 설치되지 않았습니다.\nsql/admin_tools_3.153.sql을 Supabase SQL Editor에서 실행해 주세요.", "중개사무소 정보", document.getElementById("brokerOfficeKakaoInput"));
						}
	...
	async function openAdminPage(tabKey = null)
	{
		...
		if (isAdminUser(user)) renderAdminTopbarMenu(isApprovedBrokerOffice(window.realjejuCurrentBrokerOffice));
		if (!isAdminUser(user)) {
	...
	{
		return "sql/admin_tools_3.153.sql을 Supabase SQL Editor에서 실행한 뒤 다시 시도하세요.";
	}
	...
	function updateAdminListingFilterCounts(rows)
	{
		...
		setText("adminListingsAllCount", activeRows.length);
	...
	{
		const rpcResult = await client.rpc("get_admin_property_listings");
		...
		if (!/get_admin_property_listings|PGRST202|schema cache|function/i.test(message)) {
			return rpcResult;
		}

		console.warn("관리자 매물 목록 RPC 미설치, 직접 조회로 대체:", rpcResult.error);
		return client
			.from("property_listings")
...
(function bindPropertyListingSave()
{
	...
	function collectListingPayload(status)
	{
		...
		const savedAddress = window.REALJEJU_PROPERTY_ADDRESS && typeof window.REALJEJU_PROPERTY_ADDRESS === "object"
			...
			: null;
		const locationDisplayType = addressInput?.dataset.locationDisplayType || radioValue("propertyAddressLocationMode") || "marker";
