...
function shouldUseListViewCounts()
{
	...
	const isLocalPreview = protocol === "file:" || host === "localhost" || host === "127.0.0.1" || host === "::1";
...
function getFloorDisplay(floors)
{
	...
	const currentFloor = floors.current_floor;
	const totalFloor = floors.total_floor;
	const numeric = currentFloor !== null && currentFloor !== undefined && currentFloor !== "" && String(currentFloor) === String(totalFloor)
		? `${currentFloor}층`
		: [currentFloor, totalFloor].filter(v => v !== null && v !== undefined && v !== "").join(" / ");
...
async function openDetailPanel(item, options = {})
{
	...
	detailRoadviewBtn.style.setProperty("display", detailKakaoUrl ? "inline-flex" : "none", "important");
...
function normalizeRealjejuPhone(value)
	...
	const mySuiteTabCopy = {
		...
		inquiries: ["1:1 문의내역", "문의하신 내용을 한 곳에서 확인할 수 있도록 준비중입니다."],
...
{
	loadRecommendData()
	.then(syncFeatureButtons)
	.catch(err => {
		console.error("추천 데이터 처리 실패:", err);
		syncFeatureButtons();
	});
