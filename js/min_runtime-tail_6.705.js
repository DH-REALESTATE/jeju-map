...
(function () {
	...
	function getClient() {
		...
		if (window.__realjejuSupabaseClient) return window.__realjejuSupabaseClient;
		...
		window.__realjejuSupabaseClient = fallbackSupabaseClient;
...
(function initPropertyRegisterDropdowns()
{
	...
	dropdowns.forEach((dropdown) =>
	{
		...
		const dialogCloseButton = dropdown.querySelector("[data-realjeju-mobile-filter-dialog-close]");
...
(function bindPropertyListingSave()
{
	...
	async function uploadListingPhotos(client, userId, listingId, selectedFiles = null)
	{
		const files = Array.isArray(selectedFiles)
			? selectedFiles.slice()
			: (Array.isArray(window.realjejuPropertyPhotoFiles) ? window.realjejuPropertyPhotoFiles.slice() : []);
	...
	async function saveListing(status, options = {})
	{
		...
		try {
			...
			// Capture browser-owned photo state before the published form is reset.
			const photoFilesSnapshot = Array.isArray(window.realjejuPropertyPhotoFiles)
				? window.realjejuPropertyPhotoFiles.filter(Boolean).slice()
				: [];
			const photoSaveListBuilder = typeof window.realjejuBuildPropertyPhotoSaveList === "function"
				? window.realjejuBuildPropertyPhotoSaveList
				: null;
			const pendingPhotoSlots = photoFilesSnapshot.map((unusedFile, index) => ({
				url: "__realjeju_pending_photo_" + index + "__",
				__realjejuPendingPhotoIndex: index
			}));
			const photoOrderTemplate = photoSaveListBuilder
				? photoSaveListBuilder(pendingPhotoSlots)
				: [
					...(Array.isArray(window.realjejuExistingPropertyPhotos)
						? window.realjejuExistingPropertyPhotos.filter(Boolean).map((photo) => ({ ...photo }))
						: []),
					...pendingPhotoSlots
			...
			const shouldReturnMySuiteFavorites = !shouldReturnAdminListings && !shouldReturnMapHome
				&& !!(page && page.dataset.returnToMySuiteFavorites === "1");
			const returnContext = shouldReturnAdminListings
				? "admin-listings"
				: (shouldReturnMapHome ? "map-home" : (shouldReturnMySuiteFavorites ? "my-suite-favorites" : "broker-home"));

			const persistListingPhotos = async () => {
				const uploadResult = await uploadListingPhotos(client, user.id, savedListingId, photoFilesSnapshot);
				const uploadedPhotosBySourceIndex = new Map();
				(Array.isArray(uploadResult.photos) ? uploadResult.photos : []).forEach((photo, fallbackIndex) => {
					const sourceIndex = Number(photo && photo.source_index);
					uploadedPhotosBySourceIndex.set(Number.isInteger(sourceIndex) ? sourceIndex : fallbackIndex, photo);
				});
				let savedPhotos = photoOrderTemplate
					.map((photo) => {
						const pendingPhotoIndex = Number(photo && photo.__realjejuPendingPhotoIndex);
						if (Number.isInteger(pendingPhotoIndex)) {
							const uploadedPhoto = uploadedPhotosBySourceIndex.get(pendingPhotoIndex);
							if (!uploadedPhoto) return null;
							const storedPhoto = { ...uploadedPhoto };
							delete storedPhoto.source_index;
							return storedPhoto;
						}
						return photo && typeof photo === "object" ? { ...photo } : photo;
					})
					.filter((photo) => photo && String(photo.url || photo.publicUrl || "").trim())
					.map((photo, index) => ({ ...photo, order: index + 1 }));
			...
			const refreshSavedListingAfterPhotoSave = () => {
				if (typeof window.realjejuRefreshSavedListingInMemory !== "function") return Promise.resolve(null);
				return Promise.resolve(window.realjejuRefreshSavedListingInMemory(savedListingId, {
			...
			const finishSuccessfulPropertySave = () => {
				...
				if (status === "published" && typeof window.resetPropertyRegisterFormFields === "function") {
			...
			const listingPhotoSavePromise = persistListingPhotos();
			if (status === "published") {
				window.REALJEJU_LAST_SAVED_LISTING_ID = savedListingId;
				propertyRegisterUserDirty = false;
				keepButtonsLockedUntilConfirm = true;
				listingPhotoSavePromise
					.then((uploadResult) => {
						if (uploadResult.failedCount > 0) {
							console.warn("매물 사진 " + uploadResult.failedCount + "장은 업로드되지 않았습니다.");
						}
						return refreshSavedListingAfterPhotoSave();
					})
					.catch((photoSaveError) => {
						console.error("매물 사진 백그라운드 저장 실패:", photoSaveError);
					});
			...
			try {
				uploadResult = await listingPhotoSavePromise;
			} catch (photoSaveError) {
				console.error("매물 사진 저장 실패:", photoSaveError);
				openAuthErrorModal("매물은 저장됐지만 사진 정보 저장에 실패했습니다.", "매물 등록", null);
				return;
			}
			let completeMessage = existingListingId ? "임시저장 내용이 수정되었습니다." : "임시저장되었습니다.";
...
(function bindAuthStateGoHome()
{
	...
	function goRealjejuHome(options = {})
	{
		if (isRealjejuPasswordRecoveryLocked()) {
			...
			const targetCategory = requestedCategory || "realestate";
			const currentCategory = String(document.body?.dataset?.globalCategory || "").trim();
		...
		try {
			...
			if (!usePresaleMode && currentCategory !== targetCategory && typeof syncGlobalCategoryActive === "function") {
				categoryModePromise = Promise.resolve(syncGlobalCategoryActive(targetCategory));
			}
		} catch (error) {}
...
(function initPropertyAddressInputModal(){
	...
	const quickAddressFallbackCenter = { lat: 33.3617, lng: 126.7052 };
	...
	function getAddressMapLatLngFromPointerEvent(event){
		if (!addressLocationMap || !locationMapEl || !window.kakao || !kakao.maps || typeof addressLocationMap.getProjection !== "function") return null;
		const rect = locationMapEl.getBoundingClientRect();
		const x = Number(event?.clientX) - rect.left;
		const y = Number(event?.clientY) - rect.top;
		if (!Number.isFinite(x) || !Number.isFinite(y)) return null;
		const projection = addressLocationMap.getProjection();
		if (!projection || typeof projection.coordsFromContainerPoint !== "function") return null;
	...
	// 6.273: 주소 변경이나 포인터 종료 뒤에도 지도 잠금이 남지 않도록 드래그 상태를 한 경로에서 정리한다.
	function finishAddressRectangleDrag(resetBounds = false){
		const hadActiveDrag = isAddressRectangleDragging || !!addressRectangleDragStartLatLng;
		const pointerId = addressRectangleDragPointerId;
		isAddressRectangleDragging = false;
		addressRectangleDragStartLatLng = null;
		addressRectangleDragPointerId = null;
		if (pointerId != null && locationMapEl?.hasPointerCapture?.(pointerId)) {
			try { locationMapEl.releasePointerCapture(pointerId); } catch (error) {}
		}
		if (hadActiveDrag || resetBounds) addressLocationMap?.setDraggable(true);
		if (resetBounds) {
			addressLocationBounds = null;
			addressLocationRectangle?.setMap(null);
	...
	function renderAddressLocationMap(){
		...
		finishAddressRectangleDrag(true);
	...
	function openModal(target = "property"){
		const nextAddressSearchTarget = target === "brokerOffice" || target === "brokerOfficeEdit" || target === "mySuiteBrokerApply" || target === "partTime" ? target : "property";
		// 6.273: 닫기 직전 입력칸 포커스 복원으로 같은 주소 모달이 재진입해 selectedAddress를 비우지 않게 한다.
		if (modal.classList.contains("open") && addressSearchTarget === nextAddressSearchTarget) return;
		addressSearchTarget = nextAddressSearchTarget;
	...
	// 6.273: 카카오 Map에는 안정적인 mousedown 시작 이벤트가 없어 지도 DOM 포인터를 좌표로 변환해 사각형을 이동한다.
	locationMapEl?.addEventListener("pointerdown", (event) => {
		if (event.pointerType === "mouse" && event.button !== 0) return;
		const latLng = getAddressMapLatLngFromPointerEvent(event);
		if (!latLng || getAddressLocationMode() !== "rectangle" || !isLatLngInsideAddressBounds(latLng)) return;
		addressRectangleDragPointerId = event.pointerId;
		startAddressRectangleDrag(latLng);
		try { locationMapEl.setPointerCapture(event.pointerId); } catch (error) {}
		event.preventDefault();
		event.stopPropagation();
	}, true);
	locationMapEl?.addEventListener("pointermove", (event) => {
		if (!isAddressRectangleDragging || addressRectangleDragPointerId !== event.pointerId || !addressRectangleDragStartLatLng) return;
		const latLng = getAddressMapLatLngFromPointerEvent(event);
		if (!latLng) return;
		const latDiff = latLng.getLat() - addressRectangleDragStartLatLng.getLat();
		const lngDiff = latLng.getLng() - addressRectangleDragStartLatLng.getLng();
		moveAddressRectangleByDelta(latDiff, lngDiff);
		addressRectangleDragStartLatLng = latLng;
		event.preventDefault();
		event.stopPropagation();
	}, true);
	locationMapEl?.addEventListener("pointerup", (event) => {
		if (addressRectangleDragPointerId != null && addressRectangleDragPointerId !== event.pointerId) return;
		finishAddressRectangleDrag(false);
	}, true);
	locationMapEl?.addEventListener("pointercancel", (event) => {
		if (addressRectangleDragPointerId != null && addressRectangleDragPointerId !== event.pointerId) return;
		finishAddressRectangleDrag(false);
	}, true);
	document.addEventListener("mouseup", () => finishAddressRectangleDrag(false));
	window.addEventListener("blur", () => finishAddressRectangleDrag(false));
	updateDetailHideRow();
...
(function bindPaymentPageEvents()
{
	function bind()
	{
		...
		// 6.140: 첫 지도 화면의 비로그인 사용자는 이용권 DB를 조회하지 않는다.
		// 로그인 상태이거나 결제 화면에 실제 진입한 경우에만 최신 이용권을 동기화한다.
		const shouldRefreshPaymentPlan = !!window.realjejuCurrentAuthUser?.id
			|| document.body?.classList?.contains("payment-page-open");
		if (shouldRefreshPaymentPlan && typeof window.realjejuRefreshPaymentPlanState === "function") {
...
(function () {
	...
	function applyTopbarFilterSafeLeft() {
		...
		if (sideNavToggle) {
			if (overlayRight > 0) sideNavToggle.style.display = "none";
...
(function () {
	...
	function syncTopbarPageTabsOffset()
	{
		...
		if (Number.isFinite(startLeft)) {
			tabs.style.setProperty("left", `${startLeft}px`);
			tabs.style.setProperty("right", "150px");
			tabs.style.setProperty("transform", "translateY(-50%)");
		}
	}
...
(function syncAccountPageCommonFooters()
{
	...
	// 6.140: guest map startup does not need account footer cloning or its DOM scan.
	const bindAccountFootersWhenReady = () => {
		if (window.realjejuWorkspaceReady === true) {
			replaceFooters();
			return;
		}
		document.addEventListener("realjeju:workspace-ready", replaceFooters, { once: true });
	};
	if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", bindAccountFootersWhenReady);
	else bindAccountFootersWhenReady();
})();
...
(function syncAuthAccountCommonFooter()
{
	...
	// 6.140: auth footer observers are installed only when login/account workspace is requested.
	const bindAuthFooterWhenReady = () => {
		if (window.realjejuWorkspaceReady === true) {
			bind();
			return;
		}
		document.addEventListener("realjeju:workspace-ready", bind, { once: true });
	};
	if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", bindAuthFooterWhenReady);
	else bindAuthFooterWhenReady();
})();
...
(function bindRealjejuRouteController()
{
	...
	function releaseInitialRouteGuard()
	{
		...
		initialRouteGuardReleaseFrame = window.requestAnimationFrame(() => {
			initialRouteGuardReleaseFrame = window.requestAnimationFrame(() => {
				...
				if (releaseRequestId !== latestRouteRequest || applyingRoute) {
	...
	async function openCategoryRoute(category, requestId)
	{
		...
		const shouldLoadProperties = key === "realestate"
		...
		if (shouldLoadProperties) {
			Promise.resolve().then(() => loadProperties()).catch((error) => {
				console.warn("부동산 홈 초기 매물 로드 실패:", error);
			});
		}
	...
	{
		if (typeof window.realjejuPreloadWorkspace === "function") {
			await window.realjejuPreloadWorkspace();
