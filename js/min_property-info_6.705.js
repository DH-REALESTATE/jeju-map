...
(function initParcelPropertyExperience5293()
{
  ...
  function getTradeCacheSnapshot(cache, key)
  {
    const entry = cache.get(key);
    if (!entry) return null;
    return {
      value: entry.value,
      isExpired: !Number.isFinite(entry.expiresAt) || entry.expiresAt <= Date.now()
    };
  }
  ...
  function buildOverview(feature)
  {
    ...
    return '<section class="parcel-property-overview"><div class="parcel-property-overview-copy">'
      ...
      + '<p class="parcel-property-overview-secondary">토지 <span data-parcel-property-area-sqm="' + (area == null ? "" : area) + '">' + esc(formatArea(area)) + '</span><span data-parcel-building-summary></span></p></div><button type="button" class="map-area-unit-toggle-btn parcel-land-area-unit-toggle parcel-overview-area-unit-toggle" data-parcel-land-area-unit-toggle aria-label="' + (currentAreaUnit() === "pyeong" ? "㎡" : "평") + ' 단위로 전환" title="평 / ㎡"><i class="fa-solid fa-arrows-rotate" aria-hidden="true" hidden></i><span class="map-unit-cycle-label">' + (currentAreaUnit() === "pyeong" ? "㎡" : "평") + '</span></button>'
      + '</section>';
  }

  function isNamedHousingComplexType(value)
  {
    return /아파트|연립주택|연립|다세대주택|다세대|오피스텔/.test(String(value || ""));
  }

  function normalizedHousingComplexName(value)
  {
    const name = String(value || "").trim();
    if (!name || name === "-" || /^(?:아파트|연립주택?|다세대주택?|오피스텔|공동주택|주택)$/.test(name)) return "";
    return name;
  }

  function updateOverviewComplexName(panel, name, typeText, source)
  {
    if (!panel) return false;
    const normalizedName = normalizedHousingComplexName(name);
    if (!normalizedName) return false;
    const target = panel.querySelector("[data-parcel-property-complex-name]");
    if (!target) return false;
    const rank = source === "building" ? 2 : 1;
    const currentRank = Number(target.dataset.parcelPropertyComplexRank) || 0;
    if (currentRank > rank) return false;
    const prefix = panel.querySelector("[data-parcel-property-overview-prefix]");
    target.textContent = normalizedName + " · ";
    target.hidden = false;
    if (prefix) prefix.hidden = true;
    target.dataset.parcelPropertyComplexRank = String(rank);
    return true;
  }

  function updateParcelRoadAddress(panel, value)
  {
    if (!panel) return false;
    const roadAddress = String(value || "").trim();
    if (!roadAddress) return false;
    const addressHeader = panel.querySelector(".parcel-land-info-address");
    if (!addressHeader) return false;
    let target = addressHeader.querySelector(".parcel-land-info-address-road");
    if (!target) {
      target = document.createElement("span");
      target.className = "parcel-land-info-address-road";
      addressHeader.appendChild(target);
    }
    target.textContent = roadAddress;
    return true;
  }
  ...
  function buildNavigation()
  {
    return '<nav class="parcel-property-tabs" aria-label="필지 상세 구분">'
      + '<button type="button" class="is-active" data-parcel-section-target="parcel-property-land">토지</button>'
      + '<button type="button" data-parcel-section-target="parcel-property-realtrade">실거래가</button>'
  ...
  function buildTradeShell()
  {
    return '<section id="parcel-property-realtrade" class="parcel-property-block parcel-property-realtrade">'
      + '<h2>실거래가</h2><div class="parcel-property-empty realjeju-property-state-message" data-parcel-exact-trades data-parcel-trade-loading="true">실거래 정보가 없습니다.</div>'
      + '<div class="parcel-property-subsection parcel-property-trade-graph-section is-collapsed"><h3 class="parcel-property-nearby-heading">주변 유사거래<button type="button" class="parcel-property-nearby-toggle" data-parcel-nearby-toggle aria-expanded="false">보기</button></h3><div data-parcel-trade-chart class="parcel-property-loading" hidden></div></div>'
      + '<div class="parcel-property-subsection"><h3>추천 매물</h3><div data-parcel-recommended-listing class="parcel-property-loading"></div></div>'
  ...
  {
    return '<section id="parcel-property-building" class="parcel-property-block parcel-property-building"><h2>건물 정보</h2><div data-parcel-building-content class="parcel-property-loading"></div></section>';
  }
  ...
  function clearPanelNavigationAnchor(panel)
  {
    if (!panel) return;
    clearTimeout(panel.__parcelNavigationLockTimer);
    clearTimeout(panel.__parcelNavigationAnchorStartTimer);
    if (panel.__parcelNavigationAnchorFrame) cancelAnimationFrame(panel.__parcelNavigationAnchorFrame);
    if (panel.__parcelNavigationResizeObserver) panel.__parcelNavigationResizeObserver.disconnect();
    panel.__parcelNavigationAnchorFrame = 0;
    panel.__parcelNavigationResizeObserver = null;
    panel.__parcelNavigationLockTarget = "";
  }

  function alignPanelNavigationTarget(panel, sectionId, behavior)
  {
    const section = panel && panel.querySelector("#" + sectionId);
    ...
    if (scroller && stickyHead && typeof scroller.scrollTo === "function") {
      ...
      const tabsBorderOverlap = tabs ? 1 : 0;
      const dividerHeight = Math.max(0, parseFloat(getComputedStyle(section).marginTop) || 0);
      const targetTop = scroller.scrollTop + sectionRect.top - scrollerRect.top
        - stickyHeight - tabsHeight + tabsBorderOverlap - dividerHeight;
      scroller.scrollTo({ top: Math.max(0, targetTop), behavior: behavior || "auto" });
      return true;
    }
  ...
  function startPanelNavigationAnchor(panel, sectionId)
  {
    clearPanelNavigationAnchor(panel);
    panel.__parcelNavigationLockTarget = sectionId;
    setActiveTab(panel, sectionId);
    alignPanelNavigationTarget(panel, sectionId, "smooth");

    panel.__parcelNavigationAnchorStartTimer = setTimeout(function() {
      if (panel.__parcelNavigationLockTarget !== sectionId) return;
      alignPanelNavigationTarget(panel, sectionId, "auto");
      if (typeof ResizeObserver !== "function") return;
      const section = panel.querySelector("#" + sectionId);
      const observer = new ResizeObserver(function() {
        if (panel.__parcelNavigationLockTarget !== sectionId || panel.__parcelNavigationAnchorFrame) return;
        panel.__parcelNavigationAnchorFrame = requestAnimationFrame(function() {
          panel.__parcelNavigationAnchorFrame = 0;
          if (panel.__parcelNavigationLockTarget === sectionId) {
            alignPanelNavigationTarget(panel, sectionId, "auto");
          }
        });
      });
      panel.__parcelNavigationResizeObserver = observer;
      let reachedTarget = false;
      Array.from(panel.children).forEach(function(child) {
        if (reachedTarget) return;
        observer.observe(child);
        if (child === section) reachedTarget = true;
      });
    }, 320);

    panel.__parcelNavigationLockTimer = setTimeout(function() {
      if (panel.__parcelNavigationLockTarget === sectionId) clearPanelNavigationAnchor(panel);
    }, 8000);
  }
  ...
  function bindPanelNavigation(panel)
  {
    ...
    scroller.addEventListener("scroll", function() {
      ...
      requestAnimationFrame(function() {
        ...
        let active = "parcel-property-land";
        ["parcel-property-land", "parcel-property-realtrade", "parcel-property-building", "parcel-property-auction"].forEach(function(id) {
    ...
    const releaseNavigationAnchor = function() {
      if (panel.__parcelNavigationLockTarget) clearPanelNavigationAnchor(panel);
    };
    scroller.addEventListener("wheel", releaseNavigationAnchor, { passive: true });
    scroller.addEventListener("touchstart", releaseNavigationAnchor, { passive: true });
    scroller.addEventListener("pointerdown", releaseNavigationAnchor, { passive: true });
  }
  ...
  async function loadRecommendedListing(panel, feature)
  {
    ...
    try {
      let renderer = window.realjejuRenderRecommendedListingCard;
      if (typeof renderer !== "function" && typeof window.realjejuPreloadAppFeatures === "function") {
        await window.realjejuPreloadAppFeatures();
        if (!panel.isConnected || String(panel.dataset.parcelPnu || "") !== String(feature && feature.pnu || "")) return;
        renderer = window.realjejuRenderRecommendedListingCard;
      }
      const rendered = typeof renderer === "function" ? await renderer(target, feature) : false;
      if (section) section.hidden = !rendered;
    } catch (error) {
  ...
  function parcelTradeUmdName(feature)
  {
    ...
    const leaf = String(matches[matches.length - 1] || "").trim();
    const parent = String(matches[matches.length - 2] || "").trim();
    if (/리$/.test(leaf) && /(?:읍|면)$/.test(parent)) return parent + " " + leaf;
    return leaf;
  }
  ...
  function parcelNearbyUmdName(row)
  {
    const explicit = String(row && row.umdName || "").trim();
    const sourceParts = (explicit || String(row && row.address || "").trim()).split(/\s+/).filter(Boolean);
    const administrativeParts = sourceParts.filter(function(part) {
      return /(?:읍|면|동|리)$/.test(part);
    });
    return administrativeParts.length ? administrativeParts[administrativeParts.length - 1] : explicit;
  }

  function parcelNearbyLocation(row)
  {
    const pnu = String(row && row.pnu || "").replace(/\D/g, "");
    const umdName = parcelNearbyUmdName(row);
    const jibun = normalizeJibun(row && row.jibun) || parcelJibunFromPnu(pnu);
    const shortLocation = [umdName, jibun].filter(Boolean).join(" ");
    return shortLocation || String(row && row.address || "").trim();
  }

  function isParcelNearbySaleRow(row)
  {
    if (exactTradeKindKey(row) === "sale") return true;
    const tradeHint = [
      row && row.tradeKind,
      row && row.tradeType,
      row && row.serviceType,
    ...
    if (/rent|jeonse|monthly|전세|월세|년세/.test(tradeHint)) return false;
    const amount = finite(row && row.amountManWon);
  ...
  {
    if (!target) return;
    if (!target.__parcelNearbySelectedRange) target.__parcelNearbySelectedRange = "all";
    const selectedRange = target.__parcelNearbySelectedRange;
    const selectedRadiusMeters = Number(radiusMeters) === 500 ? 500 : 1000;
    const saleRows = (Array.isArray(rows) ? rows : []).filter(isParcelNearbySaleRow);
    ...
    const controls = '<div class="parcel-trade-graph-ranges">'
      + '<button type="button" data-parcel-trade-range="5y" class="' + (selectedRange === "5y" ? "is-active" : "") + '">5년간</button>'
    ...
    let cutoff = null;
    if (selectedRange === "5y") {
      cutoff = new Date(anchor.getFullYear() - 5, anchor.getMonth(), anchor.getDate());
    ...
    const svg = '<svg class="parcel-trade-graph-svg" viewBox="0 0 ' + width + ' ' + height + '" role="img" aria-label="주변 유사거래 그래프">'
      ...
      + '</svg>';
    target.classList.remove("parcel-property-loading");
    // 주변 유사거래 목록은 같은 필지가 반복되지 않도록 묶고 항상 최대 5건을 표시합니다.
    const nearbyDisplayCount = 5;
    const recentTradeSeen = new Set();
    const dedupedTradeRows = filtered
      .slice()
      .sort(function(a, b) { return b.date - a.date; })
      .filter(function(item) {
        const row = item.row || {};
        const normalizedPnu = String(row.pnu || "").replace(/\D/g, "");
        const addressKey = [row.umdName, row.jibun, row.apartmentName || row.buildingName]
          .map(function(value) { return String(value || "").replace(/\s+/g, ""); })
          .filter(Boolean)
          .join("|");
        const coordinateKey = [row.lat, row.lng].every(function(value) { return Number.isFinite(Number(value)); })
          ? Number(row.lat).toFixed(6) + "|" + Number(row.lng).toFixed(6)
          : "";
        const parcelKey = normalizedPnu
          ? "pnu:" + normalizedPnu
          : (addressKey && String(row.jibun || "").trim()
            ? "address:" + addressKey
            : (String(row.tradeId || "").trim()
              ? "trade:" + String(row.tradeId).trim()
              : (coordinateKey ? "coordinate:" + coordinateKey : addressKey)));
        if (!parcelKey || recentTradeSeen.has(parcelKey)) return false;
        recentTradeSeen.add(parcelKey);
        return true;
      });
    const recentTradeRows = dedupedTradeRows.length <= nearbyDisplayCount
      ? dedupedTradeRows
      : (nearbyDisplayCount === 1
        ? [dedupedTradeRows[0]]
        : Array.from({ length: nearbyDisplayCount }, function(_, index) {
          const spreadIndex = Math.round(index * (dedupedTradeRows.length - 1) / (nearbyDisplayCount - 1));
          return dedupedTradeRows[spreadIndex];
        }));
    const recentTradeList = recentTradeRows.length
      ? '<div class="parcel-trade-graph-recent-list">' +
        recentTradeRows.map(function(item) {
          ...
          const location = parcelNearbyLocation(row);
  ...
  function getParcelTradeRequestPromise(cacheKey, factory)
  {
    ...
    // Attach both handlers directly. Ignoring the Promise returned by finally()
    // creates a second unhandled rejection when the shared request fails.
  ...
  function markTradeLoadingState(target, type)
  {
    ...
    if (type === "exact") {
      target.classList.add("parcel-property-empty", "realjeju-property-state-message");
    } else {
  ...
  function exactTradeDefaultKind(rows)
  {
    const sourceRows = Array.isArray(rows) ? rows : [];
    const available = EXACT_TRADE_KIND_TABS.find(function(tab) {
      return sourceRows.some(function(row) {
        return exactTradeKindKey(row) === tab.key;
      });
    });
    return available ? available.key : "sale";
  }
  ...
  function renderExactTradeExperience(target, sourceRows, cacheCoverage, sourceCancelledRows)
  {
    ...
    if (complexTrade) {
      updateOverviewComplexName(target.closest(".parcel-land-info-panel"), complexTrade.propertyName, tradePropertyLabel(complexTrade), "trade");
    }
    ...
    const unit = target.__parcelExactTradeAreaUnit === "m2"
      ? "m2"
      : (target.__parcelExactTradeAreaUnit === "py" ? "py" : (currentAreaUnit() === "pyeong" ? "py" : "m2"));
    target.__parcelExactTradeAreaUnit = unit;
  ...
  function nearbyTradeRecordKey(row)
  {
    const tradeId = String(row && row.tradeId || "").trim();
    ...
    return [
      ...
      row && row.umdName,
      row && row.jibun,
      row && row.amountManWon,
      row && row.depositManWon,
      row && row.monthlyRentManWon,
      row && row.areaM2
    ].map(function(value) { return String(value == null ? "" : value).replace(/\s+/g, ""); }).join("|");
  }

  function mergeNearbyTradeRecords(serverRows, mapRows)
  {
    const seen = new Set();
    return [].concat(Array.isArray(serverRows) ? serverRows : [], Array.isArray(mapRows) ? mapRows : []).filter(function(row) {
      const key = nearbyTradeRecordKey(row);
  ...
  async function loadTrades(panel, feature, radiusMeters, range)
  {
    ...
    graphTarget.__parcelNearbySelectedRange = selectedRange;
    ...
    const exactCacheSnapshot = getTradeCacheSnapshot(tradeCache, exactTradeCacheKey);
    const exactCachedData = exactCacheSnapshot ? exactCacheSnapshot.value : null;
    let exactData = exactCacheSnapshot && !exactCacheSnapshot.isExpired ? exactCacheSnapshot.value : null;
    ...
    try {
      ...
      if (exactCachedData) {
        exactData = exactCachedData;
        console.warn("[realjeju land-trades partial]", "재조회 실패 시 캐시된 실거래가를 사용합니다.");
      } else {
    ...
    exactCancelledRows = exactRows.filter(isCancelledTradeRow);
    exactAll = exactRows.filter(function(row) { return !isCancelledTradeRow(row); });
    exactUnavailable = !exactData || (exactQueryStatus.exact === "error" && !exactAll.length);
    if (!exactAll.length && exactUnavailable) {
    ...
    const selectNearbyGraphRows = function(records) {
      const saleCandidates = (Array.isArray(records) ? records : []).filter(function(row) {
        const isSale = isParcelNearbySaleRow(row);
    ...
    const commitNearbyGraph = function(records) {
      const graphRows = selectNearbyGraphRows(records);
      if (!graphRows.length) return false;
      ...
      graphTarget.onclick = function(event) {
        ...
        graphTarget.__parcelNearbySelectedRange = rangeButton.getAttribute("data-parcel-trade-range");
        renderParcelTradeGraph(graphTarget, graphRows, graphTarget.__parcelNearbySelectedRange, selectedRadiusMeters);
      };
      return true;
    };
    const rawMapNearbyRecords = featurePoint && typeof window.__realjejuGetParcelTradeCanvasNearbyRows === "function"
      ? window.__realjejuGetParcelTradeCanvasNearbyRows({
        lat: featurePoint.lat,
        lng: featurePoint.lng,
        radiusMeters: selectedRadiusMeters,
        limit: 100,
        excludePnu: pnu
      })
      : [];
    const selectedLegalDongCode = pnu.replace(/\D/g, "").slice(0, 10);
    const mapNearbyRecords = rawMapNearbyRecords.map(function(row) {
      const rowPnu = String(row && row.pnu || "").replace(/\D/g, "");
      const sameLegalDong = selectedLegalDongCode && rowPnu.slice(0, 10) === selectedLegalDongCode;
      return Object.assign({}, row, {
        umdName: parcelNearbyUmdName(row) || (sameLegalDong ? targetUmdName : ""),
        jibun: normalizeJibun(row && row.jibun) || parcelJibunFromPnu(rowPnu)
      });
    });
    const mapNearbyRendered = commitNearbyGraph(mapNearbyRecords);
    (async function() {
      try {
        const nearbyCacheSnapshot = getTradeCacheSnapshot(tradeCache, nearbyTradeCacheKey);
        const nearbyCachedData = nearbyCacheSnapshot ? nearbyCacheSnapshot.value : null;
        let nearbyData = nearbyCacheSnapshot && !nearbyCacheSnapshot.isExpired ? nearbyCacheSnapshot.value : null;
        if (!nearbyData) {
        ...
        const serverNearbyRecords = Array.isArray(nearbyData.nearbyRecords) ? nearbyData.nearbyRecords.slice() : [];
        const nearbyRecords = mergeNearbyTradeRecords(serverNearbyRecords, mapNearbyRecords);
        ...
        if (!commitNearbyGraph(nearbyRecords)) {
          renderParcelTradeGraph(graphTarget, [], selectedRange, selectedRadiusMeters);
          graphTarget.onclick = null;
        }
      } catch (error) {
        if (!isCurrentTask() || isTradeRequestCancelled(error)) return;
        ...
        const nearbyCachedData = nearbyCacheSnapshot ? nearbyCacheSnapshot.value : null;
        if (nearbyCachedData) {
          console.warn("[realjeju land-trades partial]", "재조회 실패 시 캐시된 주변 유사거래를 사용합니다.");
          const nearbyQueryStatus = nearbyCachedData.queryStatus && typeof nearbyCachedData.queryStatus === "object"
            ? nearbyCachedData.queryStatus
            : {};
          const serverNearbyRecords = Array.isArray(nearbyCachedData.nearbyRecords) ? nearbyCachedData.nearbyRecords.slice() : [];
          const nearbyRecords = mergeNearbyTradeRecords(serverNearbyRecords, mapNearbyRecords);
          const nearbyUnavailable = nearbyQueryStatus.nearby === "error"
            && !nearbyRecords.length;
          removeTradeLoadingState(graphTarget, "nearby");
          if (!isCurrentTask()) return;
          if (nearbyUnavailable) {
  ...
  {
    const prices = Array.isArray(rows) ? rows : [];
    if (status === "loading") return '<div class="parcel-property-loading"></div>';
  ...
  function buildingFloorContent(floors, purposeName, detailsPending)
  {
    ...
    if (!rows.length) return detailsPending
      ? '<div class="parcel-property-loading"></div>'
  ...
  function apartmentBusinessSection(record, businesses, businessStatus, allRecords)
  {
    ...
    if (businessStatus === 'loading' || !businessStatus) {
      content = '<div class="parcel-property-empty"></div>';
  ...
  function buildingUnitTable(model)
  {
    ...
    return '<div class="parcel-building-unit-subtitle">전유부</div>'
      ...
      + rows.map(function(row) {
        ...
        return '<div class="parcel-building-unit-table-row"><strong>' + esc(buildingUnitType(row.exclusivePublic)) + '</strong><span>' + esc(buildingUnitFloorLabel(row)) + '</span><strong>' + areaValue(row.area) + '</strong><span>' + esc(buildingUnitMainAttachment(row.mainAttachment)) + '</span><span>' + esc(purpose) + '</span></div>';
      }).join("") + '</div>';
  }
  ...
  function buildingUnitSectionContent(record, allRecords)
  {
    ...
    if (activeBuildingUnitPending) {
      return '<section class="parcel-building-group parcel-building-unit-group" data-parcel-building-unit-floor><h3>호별정보</h3><div class="parcel-property-loading"></div></section>';
    }
...
window.realjejuClosePropertyInfoTransientPickers = closePropertyInfoTransientPickers;
  ...
  function buildingRecordContent(record, wastewater, housingPrices, commonHousingPrices, commonHousingStatus, businesses, businessStatus, floors, allRecords, detailsPending)
  {
	...
	const permitDate = readBuildingDetail("permitDate");
	const constructionStartDate = readBuildingDetail("constructionStartDate") || readBuildingDetail("startDate");
	const useApprovalDate = readBuildingDetail("useApprovalDate") || readBuildingDetail("approvalDate");
    ...
    return '<section class="parcel-building-group parcel-building-title-register"><h3>' + esc(record.kind === "recap" ? "총괄표제부" : "표제부") + '</h3>'
      ...
      + parking
      + '<section class="parcel-building-group parcel-building-date-group"><h3>일자정보</h3>' + infoRow("허가일", esc(formatDate(permitDate))) + infoRow("착공일", esc(formatDate(constructionStartDate))) + infoRow("사용승인일", esc(formatDate(useApprovalDate))) + '</section>'
  ...
  function openBuildingRegisterModal(panel, data, records, selectedIndex, openingEvent)
  {
    ...
    if (initialCardList) {
      ...
      initialCardList.style.setProperty("scroll-snap-type", "none");
      initialCardList.style.setProperty("scroll-behavior", "auto");
      initialCardList.style.setProperty("overflow-x", "hidden");
      initialCardList.style.setProperty("overflow-anchor", "none");
      initialCardList.style.setProperty("pointer-events", "none");
      if (Number.isFinite(initialCardWidth) && initialCardWidth > 0) {
        const lockedWidth = initialCardWidth + "px";
        initialCardList.style.setProperty("width", lockedWidth);
        initialCardList.style.setProperty("min-width", lockedWidth);
        initialCardList.style.setProperty("max-width", lockedWidth);
        initialCardList.style.setProperty("flex-basis", lockedWidth);
      }
  ...
  function renderBuilding(panel, data, selectedIndex, options)
  {
    ...
    const roadAddressRecord = records.find(function(item) {
      return String(item && item.roadAddress || "").trim();
    });
    if (roadAddressRecord) updateParcelRoadAddress(panel, roadAddressRecord.roadAddress);
    const complexRecord = records.find(function(item) {
      return normalizedHousingComplexName(item && item.buildingName);
    });
    if (complexRecord) {
      updateOverviewComplexName(
        panel,
        complexRecord.buildingName,
        [complexRecord.mainPurpose, complexRecord.otherPurpose].filter(Boolean).join(" "),
        "building"
      );
    }
    ...
    if (summary) {
      ...
      summary.innerHTML = ' · 건물 ' + (summaryArea == null
        ...
        + ((record.useApprovalDate || record.approvalDate) ? ' · ' + esc(String(record.useApprovalDate || record.approvalDate).slice(0, 4)) + '년' : "");
    }
  ...
  function enhancePanel(panel)
  {
    ...
    panel.insertBefore(detachedLand, panel.querySelector("#parcel-property-realtrade"));
  ...
  document.addEventListener("click", function(event) {
    ...
    if (tab) {
      ...
      if (section) {
        ...
        startPanelNavigationAnchor(panel, requestedTarget);
      }
      return;
    }
  ...
  window.realjejuSyncParcelTradeAreaUnit = function(unit) {
    const nextUnit = unit === "py" ? "py" : "m2";
    document.querySelectorAll(".parcel-land-info-panel").forEach(function(panel) {
      const target = panel.querySelector("[data-parcel-exact-trades]");
      if (target) {
        target.__parcelExactTradeAreaUnit = nextUnit;
        if (Array.isArray(target.__parcelExactTradeRows)) {
          renderExactTradeExperience(target, target.__parcelExactTradeRows);
        }
      }
      refreshUnits(panel);
...
(() => {
  ...
  function syncPropertyEmptyStateSpacing(panel) {
    ...
    panel.querySelectorAll(messageSelector).forEach((message) => {
      // 접힌 주변 유사거래 내부 결과는 보이지 않으므로 공통 빈 상태 여백 계산에서 제외합니다.
      if (message.closest("[hidden]") || message.closest(".parcel-property-trade-graph-section.is-collapsed")) return;
...
// 6.273: 주변 유사거래는 기본 접힘이며 사용자가 보기 버튼을 눌렀을 때만 내용을 표시합니다.
(function bindParcelNearbyTradeToggle() {
  document.addEventListener("click", function(event) {
    const button = event.target && event.target.closest
      ? event.target.closest("[data-parcel-nearby-toggle]")
      : null;
    if (!button) return;
    const section = button.closest(".parcel-property-trade-graph-section");
    const content = section ? section.querySelector("[data-parcel-trade-chart]") : null;
    if (!section || !content) return;
    const expanded = button.getAttribute("aria-expanded") === "true";
    button.setAttribute("aria-expanded", expanded ? "false" : "true");
    button.textContent = expanded ? "보기" : "접기";
