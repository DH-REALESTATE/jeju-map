...
const MAP_ROADVIEW_OVERLAY_MAX_LEVEL = 5;
...
{
	if (!state.map || typeof state.map.getLevel !== "function") return false;
	const level = Number(state.map.getLevel());
	return Number.isFinite(level) && level <= MAP_ROADVIEW_OVERLAY_MAX_LEVEL;
}
...
function setMapRoadviewOverlayVisible(visible, options = {})
{
	...
	if (shouldShow && !isMapRoadviewOverlayLevelAllowed()) {
		isMapRoadviewOverlayVisible = false;
		updateMapTypeButtons();
		if (!options.silent) {
			setAddressSearchStatus(`로드뷰 길 표시는 지도 레벨 ${MAP_ROADVIEW_OVERLAY_MAX_LEVEL} 이하에서 사용할 수 있습니다.`, true);
		}
		return;
	}
...
function initMap()
{
	...
	kakao.maps.event.addListener(state.map, "zoom_changed", () => {
		state.initialRandomListActive = false;
		if (isMapRoadviewOverlayVisible && !isMapRoadviewOverlayLevelAllowed()) {
			setMapRoadviewOverlayVisible(false, { silent: true });
		}
